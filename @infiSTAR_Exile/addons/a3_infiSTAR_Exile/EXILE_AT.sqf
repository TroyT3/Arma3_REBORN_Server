...
VERSION_DATE_IS = '14-07-2018 17-15-03';
_MAIN_CODE = {
...
fn_infiSTARAdmin_createButton = {
	params['_display','_idc'];
	_ctrl = [_display,'RscButton_infiSTAR',_idc] call fnc_createctrl;
	_ctrl
};
infiSTAR_CUSTOM_RUN = [];
...
SHIFT_IS_PRESSED=false;CTRL_IS_PRESSED=false;ALT_IS_PRESSED=false;FILLMAINSTATE=0;LASTSUBBUTTON=1;BTN_GET_ITEM_SEL=1;BTN_SPAWN_VEH_SEL=0;
SortAlphaPlease = true;SortGroupsPlease = nil;SortRangePlease = nil;
infiSTAR_add_vehicles=true;
SELECTED_TARGET_PLAYER = player;
MAIN_DISPLAY_ID = -1338;LEFT_BOX_ID = 1499;RIGHT_BOX_ID = 1501;
...
fn_infiSTARAdmin_getSelectedObject = {
	_tvObject = objectFromNetId (tvData [LEFT_BOX_ID, tvCurSel LEFT_BOX_ID]);
	if(isNull _tvObject)then
	{
	...
	{
		SELECTED_TARGET_PLAYER = _tvObject;
	};
	SELECTED_TARGET_PLAYER
};
fn_infiSTARAdmin_getClr = {
...
fn_infiSTARAdmin_TraderFunctionArray = [
	[''],
...
fn_infiSTARAdmin_validateClass = {
	((configName _x) find '_Abstract' isEqualTo -1) && {
		...
		if (isClass (configFile >> 'CfgWeapons' >> ('Exile_Weapon_' + _className))) then {false} else {true};
	}
};
...
} forEach ('(call fn_infiSTARAdmin_validateClass) && {configName _x isKindOf ''allVehicles'' && !(configName _x isKindOf ''Man'')}' configClasses (configFile >> 'CfgVehicles'));
...

{
	...
	_category = _itemInformation select 1;
	if!(_category in newAllItems_CATEGORY)then{newAllItems_CATEGORY pushBackUnique _category;};
} forEach ('(call fn_infiSTARAdmin_validateClass) && {(getText(_x >> ''vehicleClass'') isEqualTo ''Backpacks'')}' configClasses (configFile >> 'CfgVehicles'));

{
	...
	_category = _itemInformation select 1;
	if!(_category in newAllItems_CATEGORY)then{newAllItems_CATEGORY pushBackUnique _category;};
} forEach ('call fn_infiSTARAdmin_validateClass' configClasses (configFile >> 'CfgWeapons'));

{
	...
	_category = _itemInformation select 1;
	if!(_category in newAllItems_CATEGORY)then{newAllItems_CATEGORY pushBackUnique _category;};
} forEach ('call fn_infiSTARAdmin_validateClass' configClasses (configFile >> 'CfgMagazines'));
...
_log = format['<infiSTAR.de> %1 - config data loaded!',time call FN_GET_TIME_TIME];diag_log _log;
if('==== Base Deleter ====' call ADMINLEVELACCESS)then
{
	...
	CCG_fnc_bdDeleteB = {
		...
		[-5,ASLtoATL CCGbdCenter,CCGbdRadius] call fn_ACR;
	...
	CCG_fnc_bdDeleteW = {
		...
		if(isNil 'CCGbdCenter')then{throw 'BASEDELETER: Center not set!';};
		if(isNil 'CCGbdRadius')then{throw 'BASEDELETER: Radius not set!';};
		[-6,ASLtoATL CCGbdCenter,CCGbdRadius] call fn_ACR;
...
fn_infiSTARAdmin_setFocus = {
	...
	ctrlSetFocus ((findDisplay MAIN_DISPLAY_ID) displayCtrl LEFT_BOX_ID);
	ctrlSetFocus ((findDisplay MAIN_DISPLAY_ID) displayCtrl RIGHT_BOX_ID);
};
MAP_POSITION_CLICKED = getPos player;
MAP_OBJECT_SELECTED = objNull;
fn_infiSTARAdmin_MouseButtonDown = {
	if(_this select 1 isEqualTo 0)then
	{
		MAP_POSITION_CLICKED = ((_this select 0) posScreenToWorld [_this select 2, _this select 3]);
		_pos = MAP_POSITION_CLICKED;
		if(!isNil 'fnc_getParachutePos')exitWith{ _pos call fnc_getParachutePos; };
		if(ALT_IS_PRESSED)exitWith{
			_veh = vehicle player;
...
fn_infiSTARAdmin_MouseButtonUp = {
	if(_this select 1 isEqualTo 0)then
	{
		...
		if(SHIFT_IS_PRESSED && !isNil'infiSTAR_dad_state')then
		{
			...
			else
			{
				...
				{
					[20,netId _obj,_pos] call fn_ACR;
				};
			};
		};
		MAP_OBJECT_SELECTED = objNull;
...
infiSTARAdmin_badPictures = ['','picturething','picturepapercar'];
fn_infiSTARAdmin_findPicture = {
	_pic = toLower(getText (configFile >> 'CfgWeapons' >> _this >> 'picture'));
	if(_pic in infiSTARAdmin_badPictures)then{ _pic = toLower(getText (configFile >> 'CfgVehicles' >> _this >> 'picture')); };
	if(_pic in infiSTARAdmin_badPictures)then{ _pic = toLower(getText (configFile >> 'CfgMagazines' >> _this >> 'picture')); };
	if(_pic in infiSTARAdmin_badPictures)then{''}else{_pic}
};
fn_infiSTARAdmin_searchRightListBox = {
	[
		_txt,
		{
			_txt = _this;
			disableSerialization;
			_ctrl = (findDisplay MAIN_DISPLAY_ID) displayCtrl RIGHT_BOX_ID;
			lbClear _ctrl;
				...
				{
					{
						...
						if(((toLower _x) find _txt > -1)||((toLower _displayName) find _txt > -1))then{
							...
							_ctrl lbSetPicture [(lbsize _ctrl)-1,_x call fn_infiSTARAdmin_findPicture];
			...
			0 call fnc_colorizeMain;
			for '_i' from 0 to 10 do{_ctrl lbAdd '';};
		}
	] execFSM 'call.fsm';
};
fn_infiSTARAdmin_searchFieldKeyUp = {
	params['_keyUpCtrl','_keyUpId','_keyUpShift','_keyUpControl','_keyUpAlt'];
	_txt = toLower(ctrlText _keyUpCtrl);
	string_infiSTARAdmin_searchLeft = _txt;
	if(FILLMAINSTATE isEqualTo 1)then
	{
		call fn_infiSTARAdmin_searchRightListBox;
	}
	else
	{
		0 call fn_infiSTARAdmin_fillPlayer;
	};
};
fnc_SearchFieldButtons = {
	...
	_btnItems buttonSetAction 'LASTSUBBUTTON = 0;FILLMAINSTATE=1;0 call fnc_fill_infiSTAR_MAIN;0 call fn_infiSTARAdmin_setFocus;0 call fn_infiSTARAdmin_changeButtonColor;';
	if!('Items spawn menu' call ADMINLEVELACCESS)then
	{
		_btnItems ctrlEnable false;
		_btnItems ctrlShow false;
	};
	_ctrlL = _display displayCtrl LEFT_BOX_ID;
	if(isNil'ctrlL_pos_orig')then{ctrlL_pos_orig = ctrlPosition _ctrlL;};
	if(isNil'ctrleditSearch_pos_orig')then{ctrleditSearch_pos_orig = ctrlPosition _editSearch;};
	if(isNil'ctrlbtnItems_pos_orig')then{ctrlbtnItems_pos_orig = ctrlPosition _btnItems;};
	_ctrleditSearch_pos = ctrleditSearch_pos_orig;
	_ctrlbtnItems_pos = ctrlbtnItems_pos_orig;
	if(FILLMAINSTATE in [0,3,4])then
	{
		...
		_btnItems buttonSetAction '
			...
			0 call fn_infiSTARAdmin_fillPlayer;
		';
		_ctrleditSearch_pos = [SafeZoneX,(_ctrleditSearch_pos select 1)-0.045,(ctrlL_pos_orig select 2),(_ctrleditSearch_pos select 3)*0.75];
		_ctrlbtnItems_pos = [SafeZoneX,(_ctrlbtnItems_pos select 1)-0.035,(ctrlL_pos_orig select 2),(_ctrlbtnItems_pos select 3)];
	};
	_editSearch ctrlSetPosition _ctrleditSearch_pos;
	_editSearch ctrlCommit 0.3;
	_btnItems ctrlSetPosition _ctrlbtnItems_pos;
	_btnItems ctrlCommit 0;
	...
	_btnVehs buttonSetAction 'LASTSUBBUTTON = 1;FILLMAINSTATE=1;0 call fnc_fill_infiSTAR_MAIN;0 call fn_infiSTARAdmin_setFocus;0 call fn_infiSTARAdmin_changeButtonColor;';
	if!(('Spawn Vehicles' call ADMINLEVELACCESS) || ('Spawn Persistent Vehicles' call ADMINLEVELACCESS))then
	{
	...
	if('Trader Menu' call ADMINLEVELACCESS)then
	{
		_btnTrader ctrlEnable _others;
		_btnTrader ctrlShow _others;
		_btnTrader buttonSetAction 'LASTSUBBUTTON = 2;FILLMAINSTATE=1;0 call fnc_fill_infiSTAR_MAIN;0 call fn_infiSTARAdmin_setFocus;0 call fn_infiSTARAdmin_changeButtonColor;';
	}
	else
	{
		_btnTrader ctrlEnable false;
		_btnTrader ctrlShow false;
		_btnTrader buttonSetAction 'systemChat ''Access not allowed'';';
	};
};
fn_infiSTARAdmin_LoadOutMenu = {
	disableSerialization;
	...
	_ctrl = [_display,44466] call fn_infiSTARAdmin_createButton;
	...
	_ctrl = [_display,44467] call fn_infiSTARAdmin_createButton;
	...
	_ctrl = [_display,44468] call fn_infiSTARAdmin_createButton;
	_ctrl ctrlSetPosition [
		0.68 * safezoneW + safezoneX,
	...
	fnc_ButtonClick_44468 = {
		...
		call fn_infiSTARAdmin_getSelectedObject;
	...
	_ctrl = [_display,44471] call fn_infiSTARAdmin_createButton;
	_ctrl ctrlSetPosition [
	...
	fnc_ButtonClick_44471 = {
		[format['player setUnitLoadout (getUnitLoadout (objectFromNetId ''%1''))',netId player],SELECTED_TARGET_PLAYER] call admin_d0_target;
	...
	_ctrl = [_display,44472] call fn_infiSTARAdmin_createButton;
	_ctrl ctrlSetPosition [
		0.81 * safezoneW + safezoneX,
		0.8,
		0.3,
	...
	fnc_ButtonClick_44472 = {
		player setUnitLoadout (getUnitLoadout SELECTED_TARGET_PLAYER);
	...
	_ctrl ctrlSetText format['SELECTED TARGET: %1',call fn_infiSTARAdmin_getSelectedObject];
	_ctrl ctrlCommit 0;
};
fn_infiSTARAdmin_addItem = {
	params[
	...
	else
	{
		_run = '
			...
			_ret = [_target, '+str _class+'] call ExileClient_util_playerEquipment_add;
			if(!_ret)then{[_target, '+str _class+'] call ExileClient_util_playerCargo_add;};
		';
		[_run, _target] call admin_d0_target;
	};
};
fnc_FULLinit = {
	disableSerialization;
	if(isNull findDisplay MAIN_DISPLAY_ID)then
	{
		...
		call fn_infiSTARAdmin_loadControls;
	};
};
fn_infiSTARAdmin_loadControls = {
	_display = findDisplay MAIN_DISPLAY_ID;
	
	_btn = [_display,1338001] call fn_infiSTARAdmin_createButton;
	...
	string_infiSTARAdmin_searchLeft = '';
	if!(FILLMAINSTATE in [1,2])then{FILLMAINSTATE = 0;};
	_display = findDisplay MAIN_DISPLAY_ID;

	_ctrlL = _display displayCtrl LEFT_BOX_ID;
	_ctrlL ctrlRemoveAllEventHandlers 'TreeDblClick';
	_ctrlL ctrlAddEventHandler ['TreeDblClick', '(call fn_infiSTARAdmin_getSelectedObject) call fn_infiSTARAdmin_startSpectate;0 call fn_infiSTARAdmin_setFocus;'];
	_ctrlL ctrlRemoveAllEventHandlers 'TreeSelChanged';
	_ctrlL ctrlAddEventHandler ['TreeSelChanged', 'call fn_infiSTARAdmin_selChangedLeft;0 call fn_infiSTARAdmin_setFocus;'];
	0 call fn_infiSTARAdmin_fillPlayer;

	_ctrlR = _display displayCtrl RIGHT_BOX_ID;
	_ctrlR ctrlRemoveAllEventHandlers 'LBDblClick';
	_ctrlR ctrlRemoveAllEventHandlers 'LBSelChanged';
	_ctrlR ctrlAddEventHandler ['LBDblClick', 'call fnc_LBDblClick_RIGHT;0 call fn_infiSTARAdmin_setFocus;'];
	_ctrlR ctrlAddEventHandler ['LBSelChanged', 'call fnc_LBSelChanged_RIGHT;0 call fn_infiSTARAdmin_setFocus;'];
	0 call fnc_fill_infiSTAR_MAIN;

	_ctrl = _display displayCtrl 2;
	_ctrl ctrlSetText format['Players loaded in: %1 of %2                 SERVER UPTIME: %3  SERVER FPS: %4  SERVER THREADS: %5          infiSTAR.de Admin Menu 14-07-2018 17-15-03 - v93',count (call fnc_get_plr),((playersNumber west)+(playersNumber east)+(playersNumber civilian)+(playersNumber resistance)),time call FN_GET_TIME_TIME,SERVER_FPS,SERVER_THREADS];

	_btnMainMenu = _display displayCtrl 20;
	_btnMainMenu buttonSetAction 'FILLMAINSTATE=0;0 call fnc_fill_infiSTAR_MAIN;0 call fn_infiSTARAdmin_setFocus;0 call fn_infiSTARAdmin_changeButtonColor;';

	_btnSpawnMenu = _display displayCtrl 21;
	_btnSpawnMenu buttonSetAction 'FILLMAINSTATE=1;0 call fnc_fill_infiSTAR_MAIN;0 call fn_infiSTARAdmin_setFocus;0 call fn_infiSTARAdmin_changeButtonColor;';

	if('==== Loadouts ====' call ADMINLEVELACCESS)then{call fn_infiSTARAdmin_LoadOutMenu;};

	call fnc_SearchFieldButtons;
	0 call fn_infiSTARAdmin_setFocus;
	0 call fn_infiSTARAdmin_changeButtonColor;

	
	(_display displayCtrl 100) ctrlAddEventHandler ['KeyUp',{ _this call fn_infiSTARAdmin_searchFieldKeyUp }];
};
fnc_colorizeMain = {
	disableSerialization;
	_ctrl = (findDisplay MAIN_DISPLAY_ID) displayCtrl RIGHT_BOX_ID;
	_lsize = lbSize RIGHT_BOX_ID;
	for '_i' from 0 to _lsize do
	{
		_lbtxt = lbtext [RIGHT_BOX_ID,_i];
...
fn_infiSTARAdmin_changeButtonColor = {
	call fnc_SearchFieldButtons;
	disableSerialization;
	_display = findDisplay MAIN_DISPLAY_ID;
	_btnMainMenu = _display displayCtrl 20;
	...
	_btnAntiHackLog buttonSetAction 'FILLMAINSTATE=3;0 call fnc_fill_HackLog;0 call fn_infiSTARAdmin_setFocus;0 call fn_infiSTARAdmin_changeButtonColor;';
	...
	_btnAdminLog buttonSetAction 'FILLMAINSTATE=4;0 call fnc_fill_AdminLog;0 call fn_infiSTARAdmin_setFocus;0 call fn_infiSTARAdmin_changeButtonColor;';
	...
	if((LASTSUBBUTTON isEqualTo 0)&&(FILLMAINSTATE isEqualTo 1))then
	{
		...
		_RSCButton33401 = [_display,33401] call fn_infiSTARAdmin_createButton;
		_RSCButton33401 ctrlSetText 'INFRONT OF TARGET';
		_RSCButton33401 ctrlSetPosition _ctrlpos;
		_RSCButton33401 buttonSetAction 'BTN_GET_ITEM_SEL = 0;0 call fn_infiSTARAdmin_changeButtonColor;';
		_RSCButton33401 ctrlCommit 0;
		_RSCButton33402 = [_display,33402] call fn_infiSTARAdmin_createButton;
		...
		_RSCButton33402 buttonSetAction 'BTN_GET_ITEM_SEL = 1;0 call fn_infiSTARAdmin_changeButtonColor;';
	...
	if((LASTSUBBUTTON isEqualTo 1)&&(FILLMAINSTATE isEqualTo 1))then
	{
		...
		{
			_ctrlpos = [0.2 * safeZoneW + safeZoneX, 0.173 * safeZoneH + safeZoneY, 0.15 * safeZoneW, 0.025 * safeZoneH];
			_RSCButton33401 = [_display,33403] call fn_infiSTARAdmin_createButton;
			_RSCButton33401 ctrlSetText 'NonPersistent';
			_RSCButton33401 buttonSetAction 'BTN_SPAWN_VEH_SEL = 0;0 call fn_infiSTARAdmin_changeButtonColor;';
			_RSCButton33401 ctrlSetPosition _ctrlpos;
			_RSCButton33401 ctrlCommit 0;
			
			_RSCButton33402 = [_display,33404] call fn_infiSTARAdmin_createButton;
			_RSCButton33402 ctrlSetText 'Persistent';
			_RSCButton33402 buttonSetAction 'BTN_SPAWN_VEH_SEL = 1;0 call fn_infiSTARAdmin_changeButtonColor;';
	...
	if(FILLMAINSTATE isEqualTo 0)then{		
		_ctrlR ctrlSetPosition [
			safeZoneX + 0.192 * safeZoneW,
			safeZoneY + 0.06 * safeZoneH,
			0.335 * safeZoneW,
			0.935 * safeZoneH
		];
		_ctrlR ctrlCommit 0.3;
	};
	if(FILLMAINSTATE isEqualTo 1)then{
		_ctrlL ctrlSetPosition [
			safeZoneX,
		...
		_ctrlR ctrlSetPosition [
			safeZoneX + 0.192 * safeZoneW,
			safeZoneY + 0.205 * safeZoneH,
			0.335 * safeZoneW,
			0.79 * safeZoneH
		];
		_ctrlR ctrlCommit 0.3;
	};
	if(FILLMAINSTATE in [3,4])then{
		_ctrlR ctrlSetPosition [
			safeZoneX + 0.192 * safeZoneW,
			safeZoneY + 0.06 * safeZoneH,
			0.8 * safeZoneW,
...
fnc_fill_HackLog = {
	...
	if!(PVAH_AHTMPBAN isEqualTo [])then
	{
		_ctrl lbAdd 'DBL-CLICK TO REMOVE';
		{
			_reason = PVAH_AHTMPBAN_REASON select _forEachIndex;
			_lbid = _ctrl lbAdd format['%1',if(_reason find _x > -1)then{_x + ' | ' + _reason}else{_reason}];
			_ctrl lbSetData [_lbid,_x];
	...
	for '_i' from 0 to 25 do {_ctrl lbAdd '';};
};
fnc_fill_AdminLog = {
	disableSerialization;
	_ctrl = (findDisplay MAIN_DISPLAY_ID) displayCtrl RIGHT_BOX_ID;
...
[
	...
	'Remove Unconscious','UnFreeze','UnRestrain',
...
infiSTAR_Toggleable =
[
	'God Mode','God Mode (no stats change)','Vehicle God Mode','MapIcons','Player ESP','Player ESP (safezone style)','AI ESP','Dead ESP','Loot ESP',
	'Vehicle Marker','Flag Marker (with radius)','Box/Safe/Container Marker','DeadPlayer Marker','Stealth / Invisible',
	'Vehboost','UnlimAmmo','noRecoil','FastFire','Lower Terrain',
...
fnc_fill_infiSTAR_MAIN = {
	...
	if(FILLMAINSTATE == 1)then
	{
		switch (LASTSUBBUTTON) do {
			...
			case 2:{
				...
				} forEach fn_infiSTARAdmin_TraderFunctionArray;
			};
	...
	if(FILLMAINSTATE == 0)then
	{
		...
		_ctrl lbAdd 'TAB - infiSTAR Custom Keybinds Menu';
		_ctrl lbAdd 'TYPE !admin in Chat to relog as player/admin';
	};
	0 call fnc_colorizeMain;
	for '_i' from 0 to 12 do {_ctrl lbAdd '';};
};
fn_infiSTARAdmin_addPlayer = {
	_nameText = format['%1 [%2 (%3)]', _x call fnc_get_exileObjName, groupId (group _x), count units (group _x)];
	if((toLower _nameText) find string_infiSTARAdmin_searchLeft > -1)then
	{
		_obj = _x;
		_playerIndex = _tv tvAdd [[], _nameText];
		_tv tvSetData [[_playerIndex], netId _obj];
			_tv tvAdd [[_playerIndex], format['UID: %1', getPlayerUID _obj]];
			_tv tvAdd [[_playerIndex], format['POS: %1:%2', (mapGridPosition _obj) select [0,3], (mapGridPosition _obj) select [3,6]]];
			_tv tvAdd [[_playerIndex], format['DISTANCE: %1m', floor(cameraOn distance _obj)]];
			_tv tvAdd [[_playerIndex], format['HP: %1%2', round((damage _obj - 1) * -100), toString[37]]];
			_tv tvAdd [[_playerIndex], format['ExileMoney: %1', _obj getVariable ['ExileMoney', 0]]];
			_tv tvAdd [[_playerIndex], format['ExileLocker: %1', _obj getVariable ['ExileLocker', 0]]];
			_tv tvAdd [[_playerIndex], format['ExileScore: %1', _obj getVariable ['ExileScore', 0]]];
			_tv tvAdd [[_playerIndex], format['WEP: %1', getText (configFile >> 'CfgWeapons' >> (weaponState _obj) select 0 >> 'displayName')]];
		
		_alpha = 1;
		_clr = _obj call fn_infiSTARAdmin_getClr;
			_tv tvSetColor [[_playerIndex],_clr];
		if(!isNull objectParent _obj)then
		{
			_veh = objectParent _obj;
			_crew = fullCrew _veh;
			_crewCount = count _crew;
			_class = typeOf _veh;
			
			_tv tvSetPicture [[_playerIndex], (typeOf (objectParent _obj)) call fn_infiSTARAdmin_findPicture];
				_vehIndex = _tv tvAdd [[_playerIndex], format['VEH: %1', getText (configFile >> 'CfgVehicles' >> _class >> 'displayName')]];
					_tv tvAdd [[_playerIndex, _vehIndex], format['HP: %1%2', round((damage _veh - 1) * -100), toString[37]]];
					_tv tvAdd [[_playerIndex, _vehIndex], format['FUEL: %1%2', round(fuel _veh) * 100, toString[37]]];
					_tv tvAdd [[_playerIndex, _vehIndex], format['CAN MOVE: %1', if(canMove _veh)then{'YES'}else{'NO'}]];
			
			_crewIndex = _tv tvAdd [[_playerIndex], format['CREW: %1%', _crewCount]];
			if(_crewCount > 0)then
			{
				{
					_x params['_unit','_seat'];
					_index = _tv tvAdd [[_playerIndex, _crewIndex], format['%1 (%2) - %3', _unit call fnc_get_exileObjName, groupId (group _unit), _seat]];
						_tv tvAdd [[_playerIndex, _crewIndex, _index], format['UID: %1', getPlayerUID _unit]];
						_tv tvAdd [[_playerIndex, _crewIndex, _index], format['HP: %1%2', round((damage _unit - 1) * -100), toString[37]]];
				} forEach _crew;
			};
		};
	};
...
fn_infiSTARAdmin_fillPlayer = {
	...
	_tv = findDisplay MAIN_DISPLAY_ID displayCtrl LEFT_BOX_ID;
	tvClear _tv;

	_MainDisplay = findDisplay MAIN_DISPLAY_ID;
	_btnSortAlpha = _MainDisplay displayCtrl 10;if(!isNil 'SortAlphaPlease')then{_btnSortAlpha ctrlSetTextColor [0,1,0,1];} else {_btnSortAlpha ctrlSetTextColor [1,1,1,1];};
	_btnSortGroups = _MainDisplay displayCtrl 11;if(!isNil 'SortGroupsPlease')then{_btnSortGroups ctrlSetTextColor [0,1,0,1];} else {_btnSortGroups ctrlSetTextColor [1,1,1,1];};
	_btnSortRange = _MainDisplay displayCtrl 12;if(!isNil 'SortRangePlease')then{_btnSortRange ctrlSetTextColor [0,1,0,1];} else {_btnSortRange ctrlSetTextColor [1,1,1,1];};

	if(!isNil'SortAlphaPlease')then
	{
		{call fn_infiSTARAdmin_addPlayer} forEach ([(call fnc_get_plr),[],{_x call fnc_get_exileObjName},'ASCEND'] call BIS_fnc_sortBy);
	};
	if(!isNil'SortGroupsPlease')then
	{
		_groups = [];
		_players = [];
		{
			_grp = group _x;
			if(_groups pushBackUnique _grp > -1)then{{_players pushBackUnique _x;	} forEach (units _grp);};
		} forEach (call fnc_get_plr);
		
		{call fn_infiSTARAdmin_addPlayer} forEach _players;
	};
	if(!isNil'SortRangePlease')then
	{
		...
		{call fn_infiSTARAdmin_addPlayer} forEach ([(call fnc_get_plr),[],{sortRangePos distance _x},'ASCEND'] call BIS_fnc_sortBy);
	};

	_tv tvAdd [[], '-----------------------------------'];
	_index = _tv tvAdd [[], '[Selected]'];_tv tvSetColor [[_index],[1,0.7,0.15,1]];
	_index = _tv tvAdd [[], '[In Vehicle]'];_tv tvSetColor [[_index],[0.047,0.502,1,1]];
	_index = _tv tvAdd [[], '[Player]'];_tv tvSetColor [[_index],[1,0.17,0.17,1]];
	_index = _tv tvAdd [[], '[Group]'];_tv tvSetColor [[_index],[1,0.95,0,1]];
	_index = _tv tvAdd [[], '[Admin]'];_tv tvSetColor [[_index],[0,1,0,1]];
	_index = _tv tvAdd [[], '[Dead Player]'];_tv tvSetColor [[_index],[1,1,1,1]];
};
fn_infiSTARAdmin_selChangedLeft = {
	_obj = call fn_infiSTARAdmin_getSelectedObject;
	if(!isNull (findDisplay MAIN_DISPLAY_ID displayCtrl 44463))then
	{
		(findDisplay MAIN_DISPLAY_ID displayCtrl 44463) ctrlSetText format['SELECTED TARGET: %1',_obj];
	};
	if(!isNull _obj && visibleMap)then
	{
		(findDisplay 12 displayCtrl 51) ctrlMapAnimAdd [1, 0.1, getPos _obj];
		ctrlMapAnimCommit (findDisplay 12 displayCtrl 51);
	};
	_obj
};
fn_infiSTARAdmin_spawnItem = {
	_target = call fn_infiSTARAdmin_getSelectedObject;
	if((BTN_GET_ITEM_SEL isEqualTo 0)&&!(FILLMAINSTATE isEqualTo 6))then
	{
		...
		[3,netId _target,_pos,_class] call fn_ACR;
	...
	{
		[_target,_class] call fn_infiSTARAdmin_addItem;
		_log = format['Spawning %1 on %2!',_click,_target call fnc_get_exileObjName];
		_log call fnc_adminLog;
		_log call FN_SHOW_LOG;
	};
};
fn_infiSTARAdmin_exileVehTrader = {
	if((player isEqualTo (vehicle player))&&((_this select 2 isEqualTo 1)))exitWith{ systemChat '<infiSTAR.de> You need to be in a Vehicle to use this!' };
...
fnc_toggleables = {
	if((_this isEqualTo 'God Mode') && ('God Mode (no stats change)' in infiSTAR_toggled_A))exitWith{ systemChat 'Please disable   God Mode (no stats change)   first.'; };
	if((_this isEqualTo 'God Mode (no stats change)') && ('God Mode' in infiSTAR_toggled_A))exitWith{ systemChat 'Please disable   God Mode   first.'; };
	switch (_this) do {
		...
		case 'Box/Safe/Container Marker':{call adminContainerMark;};
		case 'DeadPlayer Marker':{call adminDeadPlayer;};
		case 'God Mode':{call infiSTAR_A3Invulnerability;};
		case 'God Mode (no stats change)':{call infiSTAR_A3Invulnerability2;};
	...
	{
		lbSetColor [RIGHT_BOX_ID,1,[1,0,0,1]];
	...
	{
		lbSetColor [RIGHT_BOX_ID,1,[0,1,0,1]];
...
fnc_LBDblClick_RIGHT = {
	_click = lbtext[RIGHT_BOX_ID,(lbCurSel RIGHT_BOX_ID)];
	_lbData = lbData[RIGHT_BOX_ID,(lbCurSel RIGHT_BOX_ID)];
	if(_click isEqualTo '')exitWith{};
	if(_lbData in PVAH_AHTMPBAN)exitWith{
		[-667,_lbData] call fn_ACR;
		systemchat format['Removed  %1  from TempBan Variable. Might still be banned in ban(s).txt',_lbData];
		0 call fnc_fill_HackLog;0 call fn_infiSTARAdmin_setFocus;
	};
	if(FILLMAINSTATE isEqualTo 1 && LASTSUBBUTTON isEqualTo 2)exitWith
	{
		_num = parseNumber _lbData;
		if(_num isEqualTo -1337)exitWith{};
		(fn_infiSTARAdmin_TraderFunctionArray select _num) call fn_infiSTARAdmin_exileVehTrader;
	};
	...
	{
		_unit = call fn_infiSTARAdmin_getSelectedObject;
		...
		if(_click == 'Parachute Target')exitWith{[_unit] call fnc_MoveInParachute;};
	...
	switch (_click) do {
		...
		case 'clear ingame HackLog':{[0] call fnc_clearLogArray;_click call fnc_adminLog;0 call fnc_fill_HackLog;};
		case 'clear ingame AdminLog':{[1] call fnc_clearLogArray;_click call fnc_adminLog;0 call fnc_fill_AdminLog;};
		...
		case 'AdminConsole':{0 call infiSTAR_adminConsoleInit;};
		case 'DebugConsole':{0 spawn fnc_RscDisplayDebugPublic;};
		case 'Arsenal':{['Open',true] call BIS_fnc_arsenal;};
		case 'Arsenal (Mousewheel)':{ 0 call fn_addArsenalAction; };
		case 'Copy Worldspace(coords) to RPT & Chat':{0 call fnc_worldspace;};
		case 'Mass Message':{0 call fnc_mass_message;};
		case 'Spawn Ammo':{0 call infiSTAR_A3addAmmo;};
		case 'Change ViewDistance':{0 call FN_CHANGE_VIEWDISTANCE;};
		case 'Self Disconnect':{_click call fnc_adminLog;(finddisplay 46) closeDisplay 0;};
		case 'Unban (opens a field to type in a UID)':{_click call fnc_adminLog;0 call fn_infiSTAR_Unban;};
	};
	...
	_class = lbData[RIGHT_BOX_ID,(lbCurSel RIGHT_BOX_ID)];
	if(_class in ALLC_ITEMS)exitWith
	{
		call fn_infiSTARAdmin_spawnItem;
	};
	if(_class in ALL_VEHS_TO_SEARCH_C)exitWith
	{
		if!(('Spawn Vehicles' call ADMINLEVELACCESS) || ('Spawn Persistent Vehicles' call ADMINLEVELACCESS))exitWith{};
		...
		if(BTN_SPAWN_VEH_SEL isEqualTo 0)then
		{
			...
			_log call FN_SHOW_LOG;
			[0,_class,_position,_dir,netId _target,false] call fn_ACR;
	...
	if((FILLMAINSTATE == 0)||(FILLMAINSTATE == 1))then{0 call fnc_fill_infiSTAR_MAIN;};
};
fnc_LBSelChanged_RIGHT = {
	...
	if(_cfg != '')then
	{
		...
		_pic = _class call fn_infiSTARAdmin_findPicture;
...
fn_infiSTAR_admin_ToggleSpectateOverlay = {
...
fn_infiSTARAdmin_startSpectate = {
	if(!isNil'SPECTATE_THREAD')then{terminate SPECTATE_THREAD;SPECTATE_THREAD=nil;};
	SPECTATE_THREAD = _this spawn {
		disableSerialization;
		_unit = _this;
		_uid = getPlayerUID _unit;
		_log = format['SPECTATE - switchedCamera on %1(%2) - %3',_unit call fnc_get_exileObjName,_uid,typeOf (vehicle _unit)];
		_log call fnc_adminLog;
		...
		{
			_uid = getPlayerUID _unit;
			if(_uid isEqualTo '')exitWith{call fn_infiSTAR_admin_endSpectate;};
			_veh = vehicle _unit;
			...
			if(show_spectate_overlay)then
			{
				...
				_ctrlText = _ctrlText + '<br/><t align=''left'' shadow=''2'' size=''1.1'' color=''#238701''>'+_log2+'</t>';
				...
				else
				{
					...
					_pic = _type call fn_infiSTARAdmin_findPicture;
				...
				if(!isNull _ct)then
				{
					...
					_vehCT = vehicle _ct;
					if((_vehCT isKindOf 'LandVehicle') || (_vehCT isKindOf 'Air') || (_vehCT isKindOf 'Ship') || (_vehCT isKindOf 'Tank') || (_vehCT isKindOf 'Static'))then
					{
						_cwepsV = [];
						{
							if(_x find 'Horn' == -1)then
...
fnc_clearLogArray = {
	[-668,_this select 0] call fn_ACR;
	_log = 'ingame Log(s) cleared!';
	_log call FN_SHOW_LOG;
	0 call fnc_FULLinit;
};
fnc_getSteamName = {
	[9876,getPlayerUID (_this select 0)] call fn_ACR;
...
fnc_TP2ME = {
	...
	[1,netId _unit,_pos] call fn_ACR;
...
fnc_MoveInParachute = {
	...
	_log = format['Click on Map to Parachute %1..',_target call fnc_get_exileObjName];
	_log call FN_SHOW_LOG;
	
	ParachuteTarget = _target;
	fnc_getParachutePos = {
		_obj = vehicle ParachuteTarget;
		if(_obj isKindOf 'Steerable_Parachute_F')then
		{
			deleteVehicle _obj;
		}
		else
		{
			[ParachuteTarget] call fnc_EjectTargetVeh;
		};
		
		
		['
			_parachuteObject = createVehicle [''Steerable_Parachute_F'', ['+str _this+' select 0,'+str _this+' select 1,300], [], 0, ''FLY''];
			_target = objectFromNetId '+str (netId ParachuteTarget)+';
			[[_target,_parachuteObject], {(_this select 0) moveInDriver (_this select 1)}] remoteExecCall [''spawn'',_target,false];
...
fnc_get_addvalue = {
	...
	_ctrl = [_display,77766] call fn_infiSTARAdmin_createButton;
	_ctrl ctrlSetPosition [
		0.8,
		0.033 * safezoneH,
		0.3,
...
fnc_ExileMoneyRespectChange = {
	...
	[12001 + _option,netId _target,_value] call fn_ACR;
	_log call FN_SHOW_LOG;
...
fnc_freezeTarget = {
	...
	[13,netId _target,_value] call fn_ACR;
...
fnc_unconscious = {
	...
	[18,netId _target,_value] call fn_ACR;
...
fnc_restrainTarget = {
	...
	[15,netId _target,netId player,_value] call fn_ACR;
...
admin_showinfo_catch = {
	...
	delete_old_show_thread = _this spawn {
		...
		while {_timer > diag_tickTime} do
		{
			...
			_txt = _txt + format['<t align=''left'' size=''.9'' color=''#44CD00'' shadow=''1'' shadowColor=''#000000''>code: </t><t align=''left'' size=''.9'' color=''#5FBEDE'' shadow=''1'' shadowColor=''#000000''> %1</t><br/>',_pinshown];
			...
			if!(_buildRightsUids isEqualTo [])then
			{
				...
				{
					_xuid = _x;
					_xname = profileNamespace getVariable[format['last_name_%1',_xuid],'Unknown'];
			...
			_ctrl ctrlSetPosition [safeZoneX + (safeZoneW * 0.71), safeZoneY + (safeZoneH * 0.02), safeZoneW * 0.26, (safeZoneH * 0.21)+((count _buildRightsUids)*(safeZoneH * 0.0205))];
...
fnc_LowerTerrain = { if(isNil'infiSTAR_lowerT_toggle')then{infiSTAR_lowerT_toggle=true;setTerrainGrid 50;}else{infiSTAR_lowerT_toggle=nil;setTerrainGrid 12.5;}; };
...
fnc_BIS_FreeRoamCam = {
	...
	camerathread = 0 spawn (uiNamespace getvariable 'bis_fnc_camera');
};
fnc_createBillboard = {
	_textureid = _this;
	...
	[4,_texture,_textureid,_dir,_location] call fn_ACR;
...
fnc_flipVeh = {
	...
	if((!isNull _target) && {alive _target} && {_target isKindOf 'Landvehicle' || _target isKindOf 'Air' || _target isKindOf 'Ship' || _target isKindOf 'Tank'})then
	{
		if(local _target)then
		{
...
fnc_get_inputText = {
	...
	_ctrl = [_display,7337] call fn_infiSTARAdmin_createButton;
	...
	_ctrl = [_display,7338] call fn_infiSTARAdmin_createButton;
...
fnc_ATTACH_TO = {
	...
	if(!isNull _target)then
	{
		...
		if(parseNumber((netId _target) select [0,((netId _target) find ':')]) < 2)exitWith
		{
			_log = format['Can not attach local object [%1] to player',if(_class isEqualTo '')then{_target}else{_class}];
			_log call FN_SHOW_LOG;
		};
		...
		[5,netId _target,_offset,_maxHeight] call fn_ACR;
		LastAttachedObject = _target;
		
		_log = format['Attaching [%1] to player',if(_class isEqualTo '')then{_target}else{_class}];
		_log call FN_SHOW_LOG;
	}
	else
	{
...
fnc_RepairTarget = {
	...
	if(!isNull _target)then
	{
		...
		[9,netId (vehicle _target)] call fn_ACR;
	}
	else
	{
		_log = 'target does not exist';
...
infiSTAR_A3Heal = {
	[9,netId cameraOn] call fn_ACR;
	if!(cameraOn isEqualTo player)then{[9,netId player] call fn_ACR;};
...
infiSTAR_A3addAmmo = {
	...
	else
	{
		...
		
		{
			_wep = _x;
			{
				[_veh,_x] call fn_infiSTARAdmin_addItem;
		...
		A3Invulnerability = 0 spawn {
			while {true} do
			{
				if(alive player)then
				{
					...
					{
						0 call ExileClient_system_radiation_event_onPlayerDied;
...
infiSTAR_A3Invulnerability2 = {
	...
	{
		terminate A3Invulnerability2;A3Invulnerability2=nil;
		player allowDamage true;
		player removeAllEventHandlers 'HandleDamage';
		player addEventHandler ['HandleDamage', { call ExileClient_object_player_event_onHandleDamage; }];
	};
...
fnc_draw3dhandlerAI = "
	if(!isNull cameraOn)then
	{
		...
		} forEach (cameraOn nearEntities ['AllVehicles',1500]);
	};
";
...
fnc_PlayerESP_NORM_CODE = {
	...
	
	{
		...
		if(_distance < ESPDistance)then
		{
			...
			{
				_name = _x call fnc_get_exileObjName;
				_clr = _x call fn_infiSTARAdmin_getClr;
			...
			else
			{
				...
				_icon = _class call fn_infiSTARAdmin_findPicture;
				...
				if(abs(_pos2D select 0) < 1)then
				{
					...
					if(_cnt > 0)then
					{
						_num = _cnt * -1;
						{
							...
							drawIcon3D['',_unit call fn_infiSTARAdmin_getClr,_pos,.5,_height,0,_txt,1,0.03,'PuristaMedium','',true];
						} forEach _crew;
					};
...
fnc_PlayerESP_NORM = {
	...
	if(isNil'fnc_PlayerESP_NORM_ID')then
	{
		...
		_code = {
			ESPDistance = (getObjectViewDistance select 0)+100;
			plr_in_sd = [];{if(cameraOn distance _x < ESPDistance)then{plr_in_sd pushBackUnique _x;};} forEach (call fnc_get_plr);
		};
		plr_in_sd_task = [3, _code, [], true] call ExileClient_system_thread_addtask;
		fnc_PlayerESP_NORM_ID = addMissionEventHandler ['Draw3D', { if(isNull findDisplay 49)then{0 call fnc_PlayerESP_NORM_CODE;};true } ];
	}
	else
	{
		plr_in_sd = [];
...
fnc_PlayerESP_SS_CODE = {
	...
	if (!isNull cameraOn) then
	{
		...
		} forEach ((cameraOn nearEntities[['Exile_Unit_Player', 'LandVehicle', 'Ship', 'Air', 'Tank'], _espRenderRange]) - [cameraOn]);
	};
};
fnc_PlayerESP_SS = {
	...
	{
		ESPDistance = (getObjectViewDistance select 0)+100;
		fnc_PlayerESP_SS_ID = addMissionEventHandler ['Draw3D', { if(isNull findDisplay 49)then{ESPDistance call fnc_PlayerESP_SS_CODE;};true } ];
	}
	else
	{
...
fnc_draw_MapIcons = {
	...
	{
		{
			if((_x isKindOf 'Air')||(_x isKindOf 'Landvehicle')||(_x isKindOf 'Tank'))then
			{
				_veh = vehicle _x;
				_type = typeOf _veh;
				_clr = [1,1,1,1];
...
fnc_addButtons = {
	...
	{
		_y = _y + 0.0495;
		_ctrl = [_display,_i] call fn_infiSTARAdmin_createButton;
...
infiSTAR_A3MAPICONS = {
		...
		MAP_BUTTON_THREAD = 0 spawn {
			...
			while{true}do
			{
				if(visibleMap)then
				{
					...
					{
						call fnc_addButtons;
						EventHandlerDrawAdded = (findDisplay 12 displayCtrl 51) ctrlAddEventHandler['Draw','
						if(visibleMap)then
						{
				...
				else
				{
					if(!isNil'EventHandlerDrawAdded')then
					{
						...
						(findDisplay 12 displayCtrl 51) ctrlRemoveEventHandler ['Draw',EventHandlerDrawAdded];EventHandlerDrawAdded=nil;
					};
				};
				uiSleep 0.3;
			};
	...
	else 
	{
		...
		if(!isNil'EventHandlerDrawAdded')then{(findDisplay 12 displayCtrl 51) ctrlRemoveEventHandler ['Draw',EventHandlerDrawAdded];EventHandlerDrawAdded=nil;};
		call fnc_removeButtons;
	};
};
adminVehicleMarkers = {
...
adminVehicleMarker = {
	...
	{
		markadVehicleMarker = 0 spawn adminVehicleMarkers;
	}
	else
	{
...
adminContainerMark = {
	if(isNil 'CONTAINER_MARK_THREAD')then
	{
		CONTAINER_MARK_CREATED = [];
		timerForContainer = 0;
		CONTAINER_MARK_THREAD = 0 spawn {
			private ['_objects'];
			while {true} do
			{
				if(time > timerForContainer)then
				{
					timerForContainer = time + 33;
					_objects = [];
					_objects append (allMissionObjects 'Exile_Container_SupplyBox');
					_objects append (allMissionObjects 'Exile_Container_Abstract');
					_objects append (allMissionObjects 'Exile_Container_Safe');
				};
				
				{
					_markerName = format['adminContainerMarks_%1',netId _x];
					_mText = markerText _markerName;
					if(_mText isEqualTo '')then
					{
						_m = createMarkerLocal [_markerName, getPos _x];
						_m setMarkerColorLocal 'ColorPink';
						_m setMarkerTypeLocal 'selector_selectable';
						_m setMarkerSizeLocal [0.5,0.5];
						_m setMarkerTextLocal (gettext (configFile >> 'CfgVehicles' >> typeOf _x >> 'displayName'));
						CONTAINER_MARK_CREATED pushBackUnique _m;
					};
				} forEach _objects;
				
				{
					if(_x select [0,19] isEqualTo 'adminContainerMarks')then
					{
						_container = objectFromNetId (_x select [20]);
						if(isNull _container)then
						{
							deleteMarkerLocal _x;
							CONTAINER_MARK_CREATED = CONTAINER_MARK_CREATED - [_x];
						};
					};
				} forEach CONTAINER_MARK_CREATED;
				uiSleep 40;
			};
		};
	}
	...
	{
		terminate CONTAINER_MARK_THREAD;CONTAINER_MARK_THREAD=nil;
		{ deleteMarkerLocal _x; } forEach CONTAINER_MARK_CREATED;
		CONTAINER_MARK_CREATED = [];
	};
};
adminDeadPlayers =
...
adminDeadPlayer = {
	...
	{
		markadDeadPlayer = 0 spawn adminDeadPlayers;
	}
	else
	{
...
fn_doHover = {
	_height = getPos hover_obj select 2;
	if(_height < hover_height)then{hover_velocity set[2,1];}else{hover_velocity set[2,0];};
	hover_obj setVelocity hover_velocity;
};
fnc_Hover = {
	_obj = cameraOn;
	if(!local _obj)exitWith{
		_log = 'Object not local. Get in driver first!';
		_log call FN_SHOW_LOG;
	};
	hover_obj = _obj;
	hover_pos = getPos _obj;
	hover_height = getPos _obj select 2;
	hover_velocity = [0,0,0];
	if(isNil 'hovverthread')then
	{
		hover_obj setPos hover_pos;
		hovverthread = addMissionEventHandler ['Draw3D', {call fn_doHover}];
		_log = 'Now Hovering';
		_log call FN_SHOW_LOG;
	}
	else
	{
		removeMissionEventHandler ['Draw3D',hovverthread];hovverthread=nil;
...
infiSTAR_go_down = {
	...
	if(local _veh)then
	{
		...
		if(_posZ > 6)then {
			_veh setVelocity [(_vel select 0),(_vel select 1),-80];
		} else {
			if(_posZ < 10)then{_veh setVelocity [0,0,-3];};
		};
		if(isTouchingGround _veh)then{_veh setVectorUp [0,0,1];};
	};
};
infiSTAR_shortTP = {
	if(isNil'infiSTAR_TpdirectionENABLED')exitWith{['ErrorTitleAndText', ['infiSTAR.de', 'Function not enabled!']] call ExileClient_gui_toaster_addTemplateToast;false};
...
fnc_RscDisplayDebugPublic = {
	...
	waitUntil
	{
		...
		_btnSpectator buttonSetAction 'diag_log (ctrlText ((findDisplay 316000) displayCtrl 12284));((findDisplay 316000) displayCtrl 12284) ctrlSetText '''';';
		...
		{
			_btnFunctions buttonSetAction '[-662,1] call fn_ACR;';
		}
		else
		{
		...
		{
			_btnConfig buttonSetAction '[-662,2] call fn_ACR;';
		}
		else
		{
...
FN_SERVER_INFORMATION_OVERLAY = {
	...
	{
		DEBUG_OVERLAY_THREAD = 0 spawn {
			private['_timer1','_timer2','_code','_ALL','_allMissionObjects','_allMissionObjectsTypeAll','_ObjectsSimulated','_Exile_Construction','_Exile_Flag','_Vehicles','_LandVehicleAirShipStatic','_allDead','_allDeadMen','_BSF_A3XAIcount','_BSF_AIcount','_BSF_OccupationCount','_BSF_VEMFrCount','_BSF_DMS_Count','_BSF_RebornCount','_BSF_RyanZombieCount','_BSF_SMZombzCount','_DeadPlayers','_Players','_AI','_SERVERTHREADS','_FPS','_MissionRunningTime','_nearestObject','_nearestObjectHealth','_nearestObjects','_LootWeaponHolder','_GroundWeaponHolder','_WeaponHolderSimulated','_Exile_Flag150m','_Exile_Construction150m','_LootWeaponHolder150m','_GroundWeaponHolder150m','_WeaponHolderSimulated150m','_ctrlTXT'];
			disableSerialization;
			...
			while{true}do
			{
				if(time > _timer1)then
				{
					...
					_Exile_Construction = {
						_x isKindOf 'Exile_Construction_Abstract_Static' || 
						_x isKindOf 'Exile_Container_SupplyBox' || 
						_x isKindOf 'Exile_Container_Abstract' || 
						_x isKindOf 'Exile_Container_Safe'
					} count _ALL;
					_allMissionObjects = count _ALL;
					_ObjectsSimulated = {simulationEnabled _x} count _ALL;
					
					_Vehicles = count vehicles;
					_LandVehicleAirShipStatic = count ([0,0,0] nearEntities [['LandVehicle','Air','Ship','Tank','Static'], 10000000]);
				...
				if(time > _timer2)then
				{
					...
					//  End BSF Unit monitor by script
				...
				_txt = format['
<t align=''left'' size=''0.85'' color=''#44CD00''>Exile_Flags on Map: </t><t align=''left'' size=''0.85'' color=''#5FBEDE''>%1</t><br/>
<t align=''left'' size=''0.85'' color=''#44CD00''>Exile_Constructions on Map: </t><t align=''left'' size=''0.85'' color=''#5FBEDE''>%2</t><br/>
<t align=''left'' size=''0.85'' color=''#44CD00''>LootWeaponHolder on Map: </t><t align=''left'' size=''0.85'' color=''#5FBEDE''>%20</t><br/>
<t align=''left'' size=''0.85'' color=''#44CD00''>GroundWeaponHolder on Map: </t><t align=''left'' size=''0.85'' color=''#5FBEDE''>%21</t><br/>
<t align=''left'' size=''0.85'' color=''#44CD00''>WeaponHolderSimulated on Map: </t><t align=''left'' size=''0.85'' color=''#5FBEDE''>%22</t><br/>
<br/>
<t align=''left'' size=''0.85'' color=''#44CD00''>Exile_Flags in 150m: </t><t align=''left'' size=''0.85'' color=''#5FBEDE''>%3</t><br/>
<t align=''left'' size=''0.85'' color=''#44CD00''>Exile_Constructions in 150m: </t><t align=''left'' size=''0.85'' color=''#5FBEDE''>%27</t><br/>
<t align=''left'' size=''0.85'' color=''#44CD00''>LootWeaponHolder in 150m: </t><t align=''left'' size=''0.85'' color=''#5FBEDE''>%23</t><br/>
<t align=''left'' size=''0.85'' color=''#44CD00''>GroundWeaponHolder in 150m: </t><t align=''left'' size=''0.85'' color=''#5FBEDE''>%24</t><br/>
<t align=''left'' size=''0.85'' color=''#44CD00''>WeaponHolderSimulated in 150m: </t><t align=''left'' size=''0.85'' color=''#5FBEDE''>%25</t><br/>
<br/>
<t align=''left'' size=''0.85'' color=''#44CD00''>allMissionObjects: </t><t align=''left'' size=''0.85'' color=''#5FBEDE''>%4</t><br/>
<t align=''left'' size=''0.85'' color=''#44CD00''>Vehicles: </t><t align=''left'' size=''0.85'' color=''#5FBEDE''>%6</t><br/>
<t align=''left'' size=''0.85'' color=''#44CD00''>LandVehicleAirShipStatic: </t><t align=''left'' size=''0.85'' color=''#5FBEDE''>%7</t><br/>
<t align=''left'' size=''0.85'' color=''#44CD00''>ObjectsSimulated: </t><t align=''left'' size=''0.85'' color=''#5FBEDE''>%5</t><br/>
<br/>
<t align=''left'' size=''0.85'' color=''#44CD00''>Players: </t><t align=''left'' size=''0.85'' color=''#5FBEDE''>%11</t><br/>
<t align=''left'' size=''0.85'' color=''#44CD00''>DeadPlayers: </t><t align=''left'' size=''0.85'' color=''#5FBEDE''>%10</t><br/>
<t align=''left'' size=''0.85'' color=''#44CD00''>allDeadMen: </t><t align=''left'' size=''0.85'' color=''#5FBEDE''>%9</t><br/>
<t align=''left'' size=''0.85'' color=''#44CD00''>allDead: </t><t align=''left'' size=''0.85'' color=''#5FBEDE''>%8</t><br/>


<t align=''left'' size=''0.85'' color=''#44CD00''>All AI: </t><t align=''left'' size=''0.85'' color=''#5FBEDE''>%12</t>  <t align=''left'' size=''0.85'' color=''#44CD00''>   A3XAI: </t><t align=''left'' size=''0.85'' color=''#5FBEDE''>%29</t>  <t align=''left'' size=''0.85'' color=''#44CD00''>   BSF: </t><t align=''left'' size=''0.85'' color=''#5FBEDE''>%36</t><br/>
<t align=''left'' size=''0.85'' color=''#44CD00''>DMS: </t><t align=''left'' size=''0.85'' color=''#5FBEDE''>%32</t>  <t align=''left'' size=''0.85'' color=''#44CD00''>   Occupation: </t><t align=''left'' size=''0.85'' color=''#5FBEDE''>%30</t>  <t align=''left'' size=''0.85'' color=''#44CD00''>   VEMFr: </t><t align=''left'' size=''0.85'' color=''#5FBEDE''>%31</t><br/>
<t align=''left'' size=''0.85'' color=''#44CD00''>Reborn: </t><t align=''left'' size=''0.85'' color=''#5FBEDE''>%33</t>  <t align=''left'' size=''0.85'' color=''#44CD00''>Ryan Zombie: </t><t align=''left'' size=''0.85'' color=''#5FBEDE''>%34</t>  <t align=''left'' size=''0.85'' color=''#44CD00''>   SM Zombz: </t><t align=''left'' size=''0.85'' color=''#5FBEDE''>%35</t><br/>


<br/>
<t align=''left'' size=''0.85'' color=''#44CD00''>CLIENT: </t><t align=''left'' size=''0.85'' color=''#5FBEDE''> [FPS: %16|THREADS: %26]</t><br/>
<t align=''left'' size=''0.85'' color=''#44CD00''>SERVER: </t><t align=''left'' size=''0.85'' color=''#5FBEDE''>[FPS: %15|THREADS: %13]</t><br/>
<t align=''left'' size=''0.85'' color=''#44CD00''>MissionRunningTime: </t><t align=''left'' size=''0.85'' color=''#5FBEDE''>%14</t><br/>
<br/>
<t align=''left'' size=''0.85'' color=''#44CD00''>Server looptime: </t><t align=''left'' size=''0.85'' color=''#5FBEDE''>%28</t><br/>
<br/>
<t align=''left'' size=''0.85'' color=''#44CD00''>TARGET TYPE: </t><t align=''left'' size=''0.85'' color=''#5FBEDE''>%17</t><br/>
<t align=''left'' size=''0.85'' color=''#44CD00''>TARGET DISTANCE: </t><t align=''left'' size=''0.85'' color=''#5FBEDE''>%18</t><br/>
<t align=''left'' size=''0.85'' color=''#44CD00''>TARGET HEALTH: </t><t align=''left'' size=''0.85'' color=''#5FBEDE''>%19</t><br/>
				',
					_Exile_Flag,
					_Exile_Construction,
					_Exile_Flag150m,
					...
					SERVER_LOOPTIME,
					_BSF_A3XAIcount,
					_BSF_OccupationCount,
					_BSF_VEMFrCount,
					_BSF_DMS_Count,
					_BSF_RebornCount,
					_BSF_RyanZombieCount,
					_BSF_SMZombzCount,
					_BSF_AIcount
...
FN_CHANGE_VIEWDISTANCE = {
	...
	_ctrl ctrlAddEventHandler ['SliderPosChanged','
		...
		((findDisplay 999) displayCtrl 44671) ctrlSetText format[''ShadowViewDistance: %1'',_this select 1];
	'];
	_ctrl ctrlCommit 0;
	...
	OVERCAST_VALUE = overcast;
	_ctrl = [_display,'RSCText',44675] call fnc_createctrl;
	_ctrl ctrlSetPosition [0.2,0.45,1,.1];
	_ctrl ctrlSetText format['Overcast (only changes when globally executed!): %1',overcast];
	_ctrl ctrlCommit 0;
	_ctrl = [_display,'RscXSliderH',44676] call fnc_createctrl;
	_ctrl sliderSetRange [0, 1];
	_ctrl sliderSetPosition OVERCAST_VALUE;
	_ctrl ctrlSetPosition [0.2,0.525];
	_ctrl ctrlSetBackgroundColor [0.15,0.15,0.15,1];
	_ctrl ctrlRemoveAllEventHandlers 'SliderPosChanged';
	_ctrl ctrlAddEventHandler ['SliderPosChanged','
		OVERCAST_VALUE = _this select 1;
		0 setOvercast OVERCAST_VALUE;
		((findDisplay 999) displayCtrl 44675) ctrlSetText format[''Overcast: %1'',overcast];
	'];
	_ctrl ctrlCommit 0;

	FOG_VALUE = fog;
	_ctrl = [_display,'RSCText',44677] call fnc_createctrl;
	_ctrl ctrlSetPosition [0.2,0.55,1,.1];
	_ctrl ctrlSetText format['Fog: %1',FOG_VALUE];
	_ctrl ctrlCommit 0;
	_ctrl = [_display,'RscXSliderH',44678] call fnc_createctrl;
	_ctrl sliderSetRange [0, 1];
	_ctrl sliderSetPosition FOG_VALUE;
	_ctrl ctrlSetPosition [0.2,0.625];
	_ctrl ctrlSetBackgroundColor [0.15,0.15,0.15,1];
	_ctrl ctrlRemoveAllEventHandlers 'SliderPosChanged';
	_ctrl ctrlAddEventHandler ['SliderPosChanged','
		FOG_VALUE = _this select 1;
		((findDisplay 999) displayCtrl 44677) ctrlSetText format[''Fog: %1'',FOG_VALUE];
	'];
	_ctrl ctrlCommit 0;
	if(!isNil'FOG_VALUE_THREAD')then{terminate FOG_VALUE_THREAD;FOG_VALUE_THREAD=nil;};
	FOG_VALUE_THREAD = 0 spawn {
		while {!isNull ((findDisplay 999) displayCtrl 44678)} do
		{
			0 setFog FOG_VALUE;
		};
	};
	...
	RAIN_VALUE = rain;
	_ctrl = [_display,'RSCText',44679] call fnc_createctrl;
	_ctrl ctrlSetPosition [0.2,0.65,1,.1];
	_ctrl ctrlSetText format['Rain: %1',RAIN_VALUE];
	_ctrl ctrlCommit 0;
	_ctrl = [_display,'RscXSliderH',44680] call fnc_createctrl;
	_ctrl sliderSetRange [0, 1];
	_ctrl sliderSetPosition RAIN_VALUE;
	_ctrl ctrlSetPosition [0.2,0.725];
	_ctrl ctrlSetBackgroundColor [0.15,0.15,0.15,1];
	_ctrl ctrlRemoveAllEventHandlers 'SliderPosChanged';
	_ctrl ctrlAddEventHandler ['SliderPosChanged','
		RAIN_VALUE = _this select 1;
		((findDisplay 999) displayCtrl 44679) ctrlSetText format[''Rain: %1'',RAIN_VALUE];
	'];
	_ctrl ctrlCommit 0;
	if(!isNil'RAIN_VALUE_THREAD')then{terminate RAIN_VALUE_THREAD;RAIN_VALUE_THREAD=nil;};
	RAIN_VALUE_THREAD = 0 spawn {
		while {!isNull ((findDisplay 999) displayCtrl 44676)} do
		{
			0 setRain RAIN_VALUE;
		};
	};
	
	_ctrl = [_display,44681] call fn_infiSTARAdmin_createButton;
	_ctrl ctrlSetText 'CHANGE WEATHER GLOBAL';
	_ctrl ctrlSetPosition [0.2,0.775,0.4,0.033 * safezoneH];
	_ctrl ctrlCommit 0;
	_ctrl ctrlSetEventHandler['ButtonClick','[16,FOG_VALUE,OVERCAST_VALUE,RAIN_VALUE] call fn_ACR;'];
};
fnc_infiAdminKeyDown = {
	params ['_editctrl','_keycode', '_keyshift', '_keyctrl', '_keyalt'];
	SHIFT_IS_PRESSED = _keyshift;
	CTRL_IS_PRESSED = _keyctrl;
	ALT_IS_PRESSED = _keyalt;
	if(_keycode isEqualTo 0x0E)then{ if(!isNil'prevLoc')then{ [1,netId player,prevLoc] call fn_ACR;prevLoc = nil; }; };
	false
};
fnc_infiAdminKeyUp = {
	params ['_editctrl','_keycode', '_keyshift', '_keyctrl', '_keyalt'];
	...
	if(_keycode in (actionKeys 'User9'))then{ if('Arsenal' call ADMINLEVELACCESS)then{ ['Open',true] call BIS_fnc_arsenal; }; };
	false
};
...
{
	if(!isNil'infiAdminMouseButtonDown')then{(findDisplay 12 displayCtrl 51) ctrlRemoveEventHandler ['MouseButtonDown',infiAdminMouseButtonDown];infiAdminMouseButtonDown=nil;};
	infiAdminMouseButtonDown = (findDisplay 12 displayCtrl 51) ctrlAddEventHandler['MouseButtonDown','call fn_infiSTARAdmin_MouseButtonDown'];
	
	if(!isNil'infiAdminMouseButtonUp')then{(findDisplay 12 displayCtrl 51) ctrlRemoveEventHandler ['MouseButtonUp',infiAdminMouseButtonUp];infiAdminMouseButtonUp=nil;};
	infiAdminMouseButtonUp = (findDisplay 12 displayCtrl 51) ctrlAddEventHandler['MouseButtonUp','call fn_infiSTARAdmin_MouseButtonUp'];
};

_log = format['<infiSTAR.de> %1 - Menu Loaded!',time call FN_GET_TIME_TIME];systemchat _log;diag_log _log;
};
infiSTAR_MAIN_CODE = [_MAIN_CODE] call fnc_CompilableString;
/* ********************************************************************************* */
/* *********************************www.infiSTAR.de********************************* */
