...
UPDATEEMAIL='troy@troytilkens.com';
INFISTARVERSION='14-07-2018 17-15-03 - v93';
if((toLower UPDATEEMAIL) in [""])exitWith{
	diag_log "<infiSTAR.de> You need to update your infiSTAR.";
...
diag_log format['<infiSTAR.de> %1 - VERSION: 14-07-2018 17-15-03 - v93',time];
{
	_x params ['_uids'];
	{
		private _uid = _x;
...
if!(isClass (missionconfigfile >> 'RscListBox_infiSTAR'))exitWith{
	_log = format['<infiSTAR.de> %1 - infiSTAR_defines.hpp is not in your MPMission or you forgot to #include it within your description.ext   [22436x2 - 14-07-2018 17-15-03 - v93 - %2 - %3]',time,serverName,productVersion];
	diag_log _log;
	['MPMission_ERROR',_log] call FNC_A3_CUSTOMLOG;
};
if!(isClass (missionconfigfile >> 'infiSTAR_AdminMenu'))exitWith{
	_log = format['<infiSTAR.de> %1 - infiSTAR_AdminMenu.hpp is not in your MPMission or you forgot to #include it within your description.ext   [22436x2 - 14-07-2018 17-15-03 - v93 - %2 - %3]',time,serverName,productVersion];
	diag_log _log;
	['MPMission_ERROR',_log] call FNC_A3_CUSTOMLOG;
};
if!(isClass (missionconfigfile >> 'infiSTAR_CHAT'))exitWith{
	_log = format['<infiSTAR.de> %1 - infiSTAR_chat.hpp is not in your MPMission or you forgot to #include it within your description.ext   [22436x2 - 14-07-2018 17-15-03 - v93 - %2 - %3]',time,serverName,productVersion];
	diag_log _log;
	['MPMission_ERROR',_log] call FNC_A3_CUSTOMLOG;
};
if!(isClass (missionconfigfile >> 'RscListBox_infiSTAR'))exitWith{
	_log = format['<infiSTAR.de> %1 - infiSTAR_KeyBinds.hpp is not in your MPMission or you forgot to #include it within your description.ext   [22436x2 - 14-07-2018 17-15-03 - v93 - %2 - %3]',time,serverName,productVersion];
	diag_log _log;
	['MPMission_ERROR',_log] call FNC_A3_CUSTOMLOG;
};
if!(getNumber(missionconfigfile >> 'infiSTAR_AdminMenu' >> 'version') isEqualTo 3)exitWith{
	_log = format['<infiSTAR.de> %1 - infiSTAR_AdminMenu.hpp in your MPMission is outdated. Update it or infiSTAR will not load.   [22436x2 - 14-07-2018 17-15-03 - v93 - %2 - %3]',time,serverName,productVersion];
	diag_log _log;
	['MPMission_ERROR',_log] call FNC_A3_CUSTOMLOG;
};
_allowedHTMLLoadURIs = getArray(missionconfigfile >> 'CfgCommands' >> 'allowedHTMLLoadURIs');
if(!("https://*" in _allowedHTMLLoadURIs) || !("http://*" in _allowedHTMLLoadURIs))exitWith{
	_log = format["<infiSTAR.de> %1 - infiSTAR_defines.hpp in your MPMission is outdated. Update it or infiSTAR will not load.   [22436x2 - 14-07-2018 17-15-03 - v93 - %2 - %3]",time,serverName,productVersion];
	diag_log _log;
	["MPMission_ERROR",_log] call FNC_A3_CUSTOMLOG;
};
...
{
	_log = format['<infiSTAR.de> %1 - call.fsm missing in your MPmission!   [22436x2 - 14-07-2018 17-15-03 - v93 - %2 - %3]',time,serverName,productVersion];
...
_fnc_RandomGen = {
	...
	_fnc_actualGen = {
		...
		_main	= ['a','7','f','6','6','f','4','2','7','5','1','1','d','8','5','c','e','8','5','7','1','7','c','2','4','6','d','4','0','0','6','4','1'];
...
_playerObjCheckLoop = call _fnc_RandomGen;['RANDOMVAR',format['_playerObjCheckLoop: %1',_playerObjCheckLoop]] call FNC_A3_CUSTOMLOG;
...
_badWarnChatTMP = _badWarnChat;_badWarnChat = ["nigger","get cancer","infishit","\t","\r"];{_badWarnChat pushBackUnique (toLower _x);}forEach _badWarnChatTMP;
...
if!(_clientOwner isEqualTo remoteExecutedOwner)exitWith{
	...
	_log = format['%1(%2) - clientOwner [%3] != remoteExecutedOwner [%4] -> %5',_clientName,_clientUID,_clientOwner,remoteExecutedOwner,_this];
...
if(_inputOption isEqualTo 3)exitWith{
	...
	_option = _origInputArray param [0, -7, [0]];
	_input = _origInputArray param [1, 0, ['',0,[]]];
...
if(_inputOption isEqualTo 4)exitWith{
	...
	if(_inputFunctionName select [0,21] isEqualTo 'fn_infiSTAR_RE_SERVER')then{
...
if!(_token_by_uid isEqualTo _tokenreceived)exitWith{
	...
	_log = format['%1(%2) - _token_by_uid [%3] != _tokenreceived [%4] -> %5',_clientName,_clientUID,_token_by_uid,_tokenreceived,_this];
	['KICKLOG',_log] call FN_ARMA_LOG;
};
...
if!(_uid_by_token isEqualTo _clientUID)exitWith{
	...
	_log = format['%1(%2) - _uid_by_token [%3] != _clientUID [%4] -> %5',_clientName,_clientUID,_uid_by_token,_clientUID,_this];
	['KICKLOG',_log] call FN_ARMA_LOG;
};
...
if!(_clientOwnerID isEqualTo remoteExecutedOwner)exitWith{
	...
	_log = format['%1(%2) - _clientOwnerID [%3] != remoteExecutedOwner [%4] -> %5',_clientName,_clientUID,_clientOwnerID,remoteExecutedOwner,_this];
	['KICKLOG',_log] call FN_ARMA_LOG;
};
...
if(_inputArray isEqualTo [])exitWith{
	...
	_log = format['%1(%2) - _inputArray was empty -> %3',_clientName,_clientUID,_this];
	['KICKLOG',_log] call FN_ARMA_LOG;
};
...
FN_GET_OBJ_BY_OWNER = compileFinal "_playerObj = objNull;{if(((owner _x) isEqualTo _owner) && _owner > 2)exitWith{_playerObj=_x};} forEach allPlayers;_playerObj";
...
_m = missionNamespace getVariable [('UPD'+'A'+'TE'+'E'+'M'+'AI'+'L'),''];_m = _m + '&cid=11218';missionNamespace setVariable [('UPD'+'A'+'TE'+'E'+'M'+'AI'+'L'),_m];
fnc_getSteamNameIfSaved = compileFinal "missionNamespace getVariable [format['STEAM_NAME_%1',_this],'']";
call compile (""+_fn_getTempBans+" = compileFinal ""if(isNil'"+_TMPBAN+"')then{"+_TMPBAN+"=[];}else{if!("+_TMPBAN+" isEqualType [])then{"+_TMPBAN+"=[];};};"+_TMPBAN+""";");
fnc_remove_billboard = compileFinal "_vehicleID = _this getVariable ['ExileDatabaseID', -1];if(_vehicleID > -1)then{format ['deleteVehicle:%1', _vehicleID] call ExileServer_system_database_query_fireAndForget;};";
fnc_deleteObject = compileFinal "
	...
	{
		_ownerUID = _objToDelete getVariable ['ExileOwnerUID',''];
		_vehicleID = _objToDelete getVariable ['ExileDatabaseID',0];
		_ownerObj = objNull;
		{
		_playerUID = getPlayerUID _x;
		diag_log format['################ infiStar EXILE_AH _playerUID: %1 - %2',_playerUID, typeName _playerUID];
			if (_ownerUID == _playerUID) then {_ownerObj = _x}; 
		} forEach allPlayers;
		_xData = [];
		[_ownerObj, 'PlayerVehiclesUpdateResponse', [['delete', _vehicleID, '', _objToDelete, _xData]]] call ExileServer_system_network_send_to;

		_pinCode = _objToDelete getVariable ['ExileAccessCode',''];
		format['BSFVehicleUpdateLog:%1:%2:%3:%4', 'Admin', 'Deleted', _pinCode, _vehicleID] call ExileServer_system_database_query_fireAndForget;
		...
		if!(_clientUID in VOTETIME_ARR)then
		{
			...
			if((_cntAll > 0) && (_cntVoted > 0))then
			{
				...
				if(_rateAll >= VOTETIME_MVP)then
				{
					...
					_msg = ['<t size=''0.4'' font =''OrbitronLight''>'+(_txt)+'</t>',safeZoneXAbs/safeZoneWAbs/4,SafeZoneY+0.02,10,0,0,3079];
...
_startAsNormal = [];
{if(count _x > 5)then{_startAsNormal pushBackUnique _x;};} forEach _tmpstartAsNormal;
...
_bigInputArray pushBack [
	[_name,_uid,_AHKickOFF,_AHKickLog,_admin,_isNormal],
	{
		...
		{if (local _x) then { if !(getPlayerUID player isEqualTo '') then { deleteVehicle _x; }; }; } forEach (entities [['Exile_Unit_Player'], [], true, true]);
...
';
fn_infiSTAR_getInMan_WallGlitch = compileFinal fn_infiSTAR_getInMan_WallGlitch;
...
_returnVal
';
fn_infiSTAR_getOutMan_WallGlitch = compileFinal fn_infiSTAR_getOutMan_WallGlitch;
...
test_getPos_thread = 0 spawn {while {true} do{if(isNull objectParent player)then{player_pos_no_vehicle = player modelToWorldVisual [0,0,0];};uiSleep 1;};};
"; }; _asd = _asd + "
...
_c = round(random 999);
fn_infiSTAR_block_von = _c;
if!(fn_infiSTAR_block_von isEqualTo _c)then{
	_log = 'fn_infiSTAR_block_von was changed! BAN HAMMER!';
	[_name,_puid,'BAN',toArray(_log)] call _AHKickLog;
	0 call _AHKickOFF;
};
fn_infiSTAR_block_von = compileFinal([_fn_infiSTAR_block_von] call fnc_CompilableString);
...
fn_start_global_anim = ""
	...
	if(ExileIsPlayingRussianRoulette)exitWith{false};
...
"";
"; }; _asd = _asd + "
infiSTAR_channel0enabledDefault = channelEnabled 0;
infiSTAR_channel1enabledDefault = channelEnabled 1;
...
if(_admin)then
{
	if!(_ltxt isEqualTo '''')then
	{
		...
		if(_ltxt isEqualTo ''!admin'')exitWith
		{
			...
			0 spawn {uiSleep .3;(findDisplay 46)closeDisplay 0;};
		};
	};
};
if(currentChannel in [0,1,2])then{
	{
		if(_ltxt find _x > -1)exitWith{
			...
			if(isNil ''infiSTAR_disableChannelThread'')then{
				[
					''ErrorTitleAndText'',
					[
						''CHAT DISABLED'',
						''GLOBAL and SIDE chat temporarily disabled''
					]
				] call ExileClient_gui_toaster_addTemplateToast;
				infiSTAR_disableChannelThread = 0 spawn {
					0 enableChannel [false, false];
					1 enableChannel [false, false];
					_timeOut = diag_tickTime + 300;
					waitUntil {diag_tickTime > _timeOut};
					0 enableChannel infiSTAR_channel0enabledDefault;
					1 enableChannel infiSTAR_channel1enabledDefault;
					infiSTAR_disableChannelThread = nil;
				};
			};
...
{ if(isNil _x)then{missionNamespace setVariable [_x,compileFinal ''];}; } forEach _white;
{ _white pushBackUnique _x; } forEach "+str _allRandomGenVars+";
...
_fn_NR = {
private _check = missionNamespace getVariable ('h'+'_'+'d'+'0'+'n'+'e');
	...
	"; }; _asd = _asd + "
		if(alive _origunit)then
		{
			if!(player isEqualTo _origunit)then
			{
				...
				{
					_infoold = format['%1(%2) - %3, %4, %5, %6', _origunit call fnc_get_exileObjName, getPlayerUID _origunit, typeOf _origunit, netId _origunit, netId _origunit, local _origunit];
					_infonew = format['%1(%2) - %3, %4, %5, %6', player call fnc_get_exileObjName, getPlayerUID player, typeOf player, netId player, netId player, local player];
					_log = format['Player object switched. Old object: [%1] - New object: [%2]. %3m distance',_infoold,_infonew,_origunit distance player];
		...
		{
			{
				...
				{
					_var = _x;
					call _fnc_finddeeper;
				};
			} forEach (allVariables missionNamespace);
			
			_vctd = diag_tickTime + 95;
		};

		player removeAllEventHandlers 'Fired';
		player addEventHandler ['Fired', { call fn_infiSTAR_fired; }];
	...
	if(!isNull _traderdisplay)then
	{
		...
		if(!isNull _obj)then
		{
			if(!local _obj)then {
				...
				['ErrorTitleAndText',['infiSTAR.de Trading','Get in driver first']] call ExileClient_gui_toaster_addTemplateToast;
			} else {
				...
				else
				{
					...
					_containerlist lbSetCurSel (_index-1);
					_containerlist lbDelete _index;
					['ErrorTitleAndText',['infiSTAR.de Trading','Vehicle can not have passengers..']] call ExileClient_gui_toaster_addTemplateToast;
				};
			};
		};
	};
...
_fixWasteDumpDupe = {
	...
	if(!isNull _wastebtn)then{
		...
		_wastebtn ctrlAddEventHandler ['MouseButtonClick',""
			...
			if(_mode isEqualTo 2)then{closeDialog 0;true};
			"+_waste_btn_thread+" = 0 spawn {
				disableSerialization;
				_timer = time + 120;
...
_escapeMenu = {
	...
	if(isNull _display49)then { _escapefound = false; } else {
	...
	"; if(!_HIDE_FROM_PLAYERS)then{ _asd = _asd + "
		if!(serverCommandAvailable '#logout')then
		{
			...
			(_display49 displayCtrl 122) ctrlSetText 'infiSTAR KeyBinds Menu';
			(_display49 displayCtrl 122) ctrlEnable true;
			(_display49 displayCtrl 122) ctrlShow true;
			(_display49 displayCtrl 122) ctrlRemoveAllEventHandlers 'ButtonClick';
			(_display49 displayCtrl 122) ctrlAddEventHandler ['ButtonClick','0 call fn_infiSTAR_keyBinds_init;'];
		};
...
"; if(_MAIN_LOOP_CHECK)then{ _asd = _asd + "
_mainLoopStartTime = time;
_mainLoopStartPlayerObject = player;
_mainLoopIterations = 0;
"; }; _asd = _asd + "
_rcheck_requested = 0;
_playerObjectChanged = [];
while{true}do{
"; if(_MAIN_LOOP_CHECK)then{ _asd = _asd + "
	if!(_mainLoopStartPlayerObject isEqualTo player)then{
		_playerObjectChanged pushBackUnique time;
		_mainLoopStartPlayerObject = player;
	};
	private _iterations_thread = [_name, _puid, _AHKickLog, _AHKickOFF, _mainLoopStartTime, _mainLoopStartPlayerObject, _mainLoopIterations, time, _playerObjectChanged] spawn {
		params['_name', '_puid', '_AHKickLog', '_AHKickOFF', '_mainLoopStartTime', '_mainLoopStartPlayerObject', '_mainLoopIterations', '_startTime', '_playerObjectChanged'];
		_i = 0;waitUntil {_i = _i + 1;sleep 1;_i > 60};
		_log = format['MAIN LOOP STOPPED! started: %1, object: %2, netId object: %3, iterations: %4, time check started: %5, object changes: %6',
		_mainLoopStartTime,_mainLoopStartPlayerObject,netId _mainLoopStartPlayerObject,_mainLoopIterations,_startTime,_playerObjectChanged];
		[_name, _puid,'SLOG',toArray(_log)] call _AHKickLog;
		
		diag_log 'MAIN LOOP STOPPED - START DIAG';
		diag_log 'active threads:';
		{diag_log str _x;} forEach diag_activeSQFScripts;
		diag_log 'MAIN LOOP STOPPED - END DIAG';
	};
	_mainLoopIterations = _mainLoopIterations + 1;
"; }; _asd = _asd + "
	call _loopCode;
	call _escapeMenu;
	call _fixTraderDupe;
	call _fixWasteDumpDupe;
	uiSleep .65;
	if(isNull player)then{
		(findDisplay 46)closeDisplay 0;
	} else {
		call _closeLockedDoors;
		if!(player getVariable ["+str _rcheckclient+",false])then{
			_rcheck_requested = _rcheck_requested + 1;
			if(_rcheck_requested > 35)then
			{
				_rcheck_requested = 0;
...
"; if(_MAIN_LOOP_CHECK)then{ _asd = _asd + "
	terminate _iterations_thread;_iterations_thread=nil;
"; }; _asd = _asd + "
};
...
_iddchecks = {
	if(dialog || {diag_tickTime - _lastEmpty > 25})then
	{
		_lastEmpty = diag_tickTime;
		_checkedDisplays = ['Display #24Control #101','Display #-1340Control #1401','Display #-1340Control #1501'];
	};
	if(_wasclosed)then
	{
			...
			{
				{
					...
					_index = _checkedDisplays pushBackUnique _checkifIn;
					if(_index > -1)then
					{
						...
						if(_controltype isEqualTo 5)then
						{
							...
							if(_size > 0)then
							{
								for '_i' from 0 to (_size-1) do
								{
									...
									_txtclean = (_lbtxt) call fn_infiSTAR_cleanString;
						...
						else
						{
							if(_controltype isEqualTo 12)then
							{
								...
								_txtclean = (_tvtxt) call fn_infiSTAR_cleanString;
							...
							else
							{
								if!(_controltype in [3,4,8,9,15,42,81,101,102])then
								{
									...
									_txtclean = (_ctrlTxt) call fn_infiSTAR_cleanString;
	...
	if(!isNil '_uiNamespace_dynamicText')then
	{
		...
		if!(_allCtrls isEqualTo [])then
		{
			...
			{ _whiteListed pushBack (_x call fnc_get_exileObjName); } forEach allPlayers;
			{
				_ctrl = _x;
				_txt = ctrlText _ctrl;
				if!(_txt isEqualTo _lastText)then
				{
...
_bigInputArray pushBack [
	[_name,_uid,_AHKickOFF,_AHKickLog],
	{
		...
		_fn_punish_glitch = {
			"; if(_wall_glitch_revert)then{ _asd = _asd + "
			"; if(_AntiTeleport)then{ _asd = _asd + "
			if(player distance _last_no_glitch_pos < 5)then{
			"; }; _asd = _asd + "
			...
			if(_timedif > 8 && _count_glitches > 4)then
			{
				_lastglitch = time;
				_count_glitches = 0;
				...
				_lineIntersectsObjs = lineIntersectsObjs [_x select 0, _x select 1];
				{
					...
					if(
						(
							...
							(_x isKindOf 'AbstractConstruction') || 
							(_x isKindOf 'AbstractWall')
						) && !(_type in ['Exile_Construction_ConcreteStairs_Static','Exile_Construction_WoodStairs_Static','Exile_Plant_GreenBush','EBM_Metalwall_stairs','EBM_Brickwall_stairs','Land_GH_Stairs_F'])
					)exitWith {
						_goodpos = false;
		...
		if(isNil 'ExileCurrentBreachingObject')then{ExileCurrentBreachingObject=objNull;};
		while{true}do{if(call fnc_check_if_enemy_base)then{if(isNull ExileCurrentBreachingObject)then{call _fnc_wall_glitch_object;};}else{uiSleep 1.5;};uiSleep .1;};
	}
];
"; }; _asd = _asd + "
...
_bigInputArray pushBack [
	[_name,_uid,_AHKickOFF,_AHKickLog],
	{
		...
		private ['_playerObject','_object','_oldObject','_class','_oldClass','_curPos','_oldPos',
		'_timeDiff','_lastTime','_speedCalc','_maxSpeedCalc','_maxSpeed','_tpCounter','_allowedD',
		'_ropeAttachedToObj','_exitHere','_allowedTeleport','_velocity',
		'_distance','_AHpos','_timer'];
		
		
		_allowedTeleport = "+str _allowedZones+";
		_XtoYpositions = "+str _XtoYpositions+";
		{if((markerType _x) in ['ExileTraderZone','ExileTraderZoneIcon','ExileBoatTraderIcon'])then{_allowedTeleport pushBack [markerPos _x,225];};} forEach allMapMarkers;
		_fn_checkTp = {
			...
			_attachedToObj = attachedTo _object;
			if(!isNull _attachedToObj)then
			{
				if(isPlayer (driver _attachedToObj))then
				{
					_exitHere = true;
				};
			};
			...
			{
				_x params[['_mpos',[]],['_msize',250]];
				if((_curPos distance2D _mpos < _msize) && (_oldPos distance2D _mpos < _msize))exitWith{_exitHere = true;};
			} forEach _allowedTeleport;
			if(_exitHere)exitWith{false};

		"; if!(_XtoYpositions isEqualTo [])then{ _asd = _asd + "
			_oldPosGood = false;
			{
				_xposes = _x;
				{
					_pos1 = _x;
					if(_oldPos distance2D _pos1 < 15)exitWith{_oldPosGood = true;};
				} forEach _xposes;
			} forEach _XtoYpositions;
			if(_oldPosGood)then
			{
				{
					_xposes = _x;
					{
						_pos2 = _x;
						if(_curPos distance2D _pos2 < 15)exitWith{_exitHere = true;};
					} forEach _xposes;
				} forEach _XtoYpositions;
			};
			if(_exitHere)exitWith{false};
		...
		while {true} do
		{
			...
			_AHpos = player getVariable ['"+_AHpos+"',[]];
			player setVariable ['"+_AHpos+"',[]];
			if(_AHpos isEqualType [])then
			{
				if(_AHpos isEqualTo [])then
				{
					if(local _object && _oldObject isEqualTo _object)then
					{
						...
						if(_distance > 9.5)then {
							_timeDiff = (diag_tickTime - _lastTime) max 0.2;
							_speedCalc = round((_distance / _timeDiff)*3.65);
							
							_oldClass = _class;
							_class = typeOf _object;
							
							_maxSpeed = if(isNull objectParent player)then{25}else{getNumber(configfile >> 'CfgVehicles' >> _class >> 'maxSpeed') max 100};
							_maxSpeedCalc = (if(diag_fps > 20)then{_maxSpeed * 2.2}else{_maxSpeed * 3.55}) max _maxSpeedCalc;
...
_bigInputArray pushBack [
	[_name,_uid,_AHKickOFF,_AHKickLog,_badIDDsToKick,_badIDDsToClose,_allowedIDDs,_admin,_iddchecks],
	{
		...
		disableSerialization;
		if(_admin)then{
			_badIDDsToKick = _badIDDsToKick - [-1341];
			_badIDDsToClose = _badIDDsToClose - [-1341];
			_allowedIDDs pushBackUnique -1341;
		};
"; if(_UDW)then{ _asd = _asd + "
		{_badIDDsToKick = _badIDDsToKick - [_x];_badIDDsToClose = _badIDDsToClose - [_x];} forEach _allowedIDDs;
...
_bigInputArray pushBack [
		...
		if(!isNull _display49)then
		{
			...
			if((_display49 displayCtrl 3307182) isEqualTo controlNull)then
			{
				[
					'',
					{
						if(!isNil'START_LOADING_ESCHTML')then{terminate START_LOADING_ESCHTML;};
						START_LOADING_ESCHTML = 0 spawn {
							disableSerialization;
	...
	{
		[0.4, _this, [], true] call ExileClient_system_thread_addTask;
	}
];
"; }; _asd = _asd + "
...
_bigInputArray pushBack [
	...
	{
		waitUntil{getClientStateNumber >= 10 && !isNull findDisplay 46 && !(getPlayerUID player isEqualTo '')};
		params[['_name',''],['_puid',''],['_AHKickLog',{REPORT_SERVER = [_this];publicVariableServer 'REPORT_SERVER'}],['_AHKickOFF',{(findDisplay 46)closeDisplay 0;}]];
...
_bigInputArray pushBack [
	[_name,_uid,_AHKickLog,_AHKickOFF,_admins,_admin,_isNormal,_server_classes],
	{
		...
		missionNamespace setVariable ['"+_AHKickOFF+"',compileFinal([_AHKickOFF] call fnc_CompilableString)];
		...
		missionNamespace setVariable ['"+_AHKickLog+"',compileFinal([_AHKickLog] call fnc_CompilableString)];
...

		{
			...
			if(_bvc isEqualType '')then
			{
				...
				_log = format['Possible injector found: %1 (%2) VALUE: [%3]', _x, typeName _bvc, str _bvc];
				[_name,_puid,'HLOG_SKICK',toArray(_log)] call _AHKickLog;
				0 call _AHKickOFF;
			};
		} forEach [
		...
		_devLog = format['infiSTAR.de AHAT %1 - 14-07-2018 17-15-03 - v93 - server running: %2:22436x2',VERSIONCHECKRESULT,time call FN_GET_TIME_TIME];diag_log _devLog;
		"; if(!_HIDE_FROM_PLAYERS)then{ _asd = _asd + "
...
if(isNil'infiSTAR_onEventsCheck_cfgNames')then
{
	...
	infiSTAR_onEventsCheck_cfgNames = _cfgNames;
	infiSTAR_onEventsCheck_onEventsArray = _onEventsArray;
};

[
	[_name,_puid,_AHKickLog,_AHKickOFF,_onEvents,infiSTAR_onEventsCheck_cfgNames,infiSTAR_onEventsCheck_onEventsArray],
	{
		...
		{
			_configName = _x;
			_currentOnEventServer = _onEventsArray param [_forEachIndex, [], [[]]];
			
			{
				_string = getText(configFile >> _configName >> _x);
				_currentStringServer = toString(_currentOnEventServer param [_forEachIndex, [], [[]]]);
				if!(_currentStringServer isEqualTo '')then
				{
					if(_string in ['',_currentStringServer])exitWith{};
					_log = format['onEventsCheck (HANDLE CHANGED): %1 changed %2 -> [%3] != [%4]',_x,_configName,_string,_currentStringServer];
					[_name,_puid,'HLOG_SKICK',toArray(_log)] call _AHKickLog;
					0 call _AHKickOFF;
				};
			} forEach _onEvents;
...
_fnc_l_on_c = {
	...
	if(_owner > 2)then
	{
		_AHKickOFF = compile('findDisplay 46 closeDisplay 0;0 spawn {uiSleep 1;AH_KICK_'+_uid+' = ''BYE'';publicVariableServer''AH_KICK_'+_uid+''';};');
		...
		if(!_isNormal)then
		{
			...
			_owner publicVariableClient 'allSupportBoxesNames';
			_owner publicVariableClient 'infiSTAR_customFunctions';
			_owner publicVariableClient 'pathToCustomBillBoardTextures';
...
_server_setToken={
	...
	missionNamespace setVariable [format['"+_token_by_uid+"%1',_this],_token];
	missionNamespace setVariable [format['"+_uid_by_token+"%1',_token],_this];
	_token
};
...
FNC_A3_infiSTARBAN = {
	...
	_owner call fn_infiSTAR_BanNotice;
	_banned = false;
	if(count _uid isEqualTo 17)then{
		_banned = format['#exec ban %1',str _uid] call fn_serverCommand;
	} else {
		if(_owner > 2)then{
			_banned = format['#exec ban %1',str _owner] call fn_serverCommand;
		};
	};
	_log = if(_reason isEqualTo '')then{format['%1(%2 - %3): %4',_name,_uid,_owner,_banned]}else{format['%1(%2 - %3) banned: %4, reason: %5',_name,_uid,_owner,_banned,_reason]};
...
fn_infiSTAR_checkGlobalBanState = {
	...
	"; if(_CHECK_BattlEye_GLOBALBANS)then{ _asd = _asd + "
	_res = ['armalog' callExtension format['a%1',_uid]] param [0,'0',['']];
	"; }else{ _asd = _asd + "
	_res = ['armalog' callExtension format['5%1',_uid]] param [0,'0',['']];
	"; }; _asd = _asd + "

	private _infiSTAR_Ban = _res select [0,1];
	if(!isNil'_infiSTAR_Ban')then
	{
		if(_infiSTAR_Ban isEqualTo '1')then
		{
			"; if(_USE_GLOBAL_BAN_CHECK)then{ _asd = _asd + "
			[_name,_uid,_owner,'GLOBAL infiSTAR BAN'] call FNC_INFISERVERKICK;
			"; }else{ _asd = _asd + "
	...
	"; if(_CHECK_BattlEye_GLOBALBANS)then{ _asd = _asd + "
	private _BattlEye_Ban = _res select [2];
	if(!isNil'_BattlEye_Ban')then
	{
		if!(_BattlEye_Ban isEqualTo '')then
		{
			_BattlEye_Ban = parseSimpleArray _BattlEye_Ban;
			_log = format['%1(%2) | BattlEye Banned on %3 -> %4',_name,_uid,_BattlEye_Ban select 2,_BattlEye_Ban select 3];
			['SURVEILLANCELOG',_log] call FNC_A3_CUSTOMLOG;
			_log call fnc_add_survlog;
			
			[_name,_uid,_owner,format['GLOBAL BAN: %1 (%2)',_BattlEye_Ban select 3,_BattlEye_Ban select 2]] call FNC_INFISERVERKICK;
		};
	};
	"; }; _asd = _asd + "
};
...
_fnc_infiSTAR_PlayerLog = {
	...
	if(_name find ':' > -1)then{
		[_name,_uid,_owner,'Bad Playername : is a bad character! it is used as seperator for extDB calls'] call FNC_INFISERVERKICK;
	};

	_lname = toLower _name;
	if('somethingwentwrong' in [_lname, toLower _uid])then{[_name,_uid,'SLOG_SKICK',toArray('Name and/or UID not allowed!')] call "+_fnc_server_handle_mpmessage+";};
...
_b = {0 spawn {sleep 20;waitUntil {time > 20};diag_log "<infiSTAR.de> license is blocked.";diag_log "<infiSTAR.de> if you did not do anything wrong. please open a support ticket on http://support.infiSTAR.de";diag_log "<infiSTAR.de> infiSTAR will NOT START when the license is blocked!";0 spawn {sleep 1;failMission "LOSER";endMission "LOSER";forceEnd;};if(!isNil"FN_GET_SERVERPW")then{(call FN_GET_SERVERPW) serverCommand "#shutdown";};};};
if("infiSTAR" != ("i" +"n" +"f" +"i" +"S" +"T" +"A" +"R"))then{0 call _b;};if("infiSTAR.de" != ("i" +"n" +"f" +"i"+"S" +"T" +"A"+"R"+".d"+"e"))then{0 call _b;};
...
"; if(_AntiTeleport)then{ _asd = _asd + "
	if(_what isEqualTo 'ADMINTP')exitWith
	{
		...
		if((_AHpos isEqualTo []) || (_oldPos isEqualTo []))exitWith{};
		...
		if(_distance > 10)then
		{
			...
			(group (vehicle _unit)) setGroupOwner 2;
...
"; }; _asd = _asd + "
	...
	"; }; _asd = _asd + "
	"; if(_disconnect_dupe_check)then{ _asd = _asd + "
	if(_what isEqualTo 'DCHECK')exitWith{"+_antidupePVResVar+" = diag_tickTime;remoteExecutedOwner publicVariableClient '"+_antidupePVResVar+"';};
	"; }; _asd = _asd + "
	...
	_log = format['%1(%2) | %3 | %4 [14-07-2018 17-15-03 - v93]',_name,_uid,_log,time call FN_GET_TIME_TIME];
...
"; if(_AntiTeleport)then{ _asd = _asd + "
	if(_what isEqualTo 'ANTI_TP')exitWith
	{
		...
		_varname = format['tpCounter_%1',_uid];
		_tpCounter = uiNameSpace getVariable [_varname,0];
		_tpCounter = _tpCounter + 1;
		uiNameSpace setVariable [_varname,_tpCounter];
		
		if(_tpCounter > 5)then
		{
			_newLog = format['%1(%2) - Player was caught teleporting %3 times! (BANNED!)',_name,_uid,_tpCounter];
			[_name,_uid,_owner,_newLog] call FNC_A3_infiSTARBAN;
			['ANTI_TP',_newLog] call FN_ARMA_LOG;
			_newLog call fnc_add_hacklog;
		};
	};
"; }; _asd = _asd + "
...
"; }; _asd = _asd + "
	if(_option isEqualTo 69)exitWith
	{
		...
		if(_opt isEqualTo 2)exitWith{
			...
			if(_owner > 2)then{['',_code,_owner,false] call FN_infiSTAR_S;};
		};
	};
	...
	{
		_target = objectFromnetId (_inputArray select 1);
		if(isNull (group _target) || isPlayer _target)then{ _target setOwner 2; } else { (group _target) setGroupOwner 2; };
	...
	if(_option isEqualTo 0)exitWith
	{
		...
		"; if(((_VehicleWhiteList_check)||(_ForbiddenVehicles_check))&&(_UVC_adminspawn))then{ _asd = _asd + "
		private _return = _vehicleClass call "+_fnc_check_type_allowed+";
		if(!_return)exitWith{
			[
		...
		if(_persistent)then
		{
			if(_pinCode isEqualTo '')then
			{
				...
				if(_pinCode isEqualTo '')then{(entities 'Tank') call _fn_getOldPincode;};
			...
			_vehicleObject setVariable ['ExileAccessCode', _pinCode,true];
			_vehicleObject addEventHandler ['GetOut', {_this call ExileServer_object_vehicle_event_onGetOut}];
			
			_vehicleObject setVariable ['ExileOwnerUID', _targetUID, true];
			_vehicleObject setVariable ['ExileIsLocked',0,true];
		    _vehicleObject setVehicleLock 'UNLOCKED';
			_vehicleObject call ExileServer_object_vehicle_database_insert;
			_vehicleObject call ExileServer_object_vehicle_database_update;
	
			_playerName = _target getVariable ['ExileName',''];
			_vehicleID = _vehicleObject getVariable ['ExileDatabaseID', -1];
			_vehicleName = VehicleNameMap getOrDefault [typeOf _vehicleObject, 'Name Not Available'];
			format['BSFVehiclePurchaseLog:%1:%2:%3:%4:%5:%6', _vehicleID, _vehicleClass, _targetUID, _playerName, _pinCode, _vehicleName] call ExileServer_system_database_query_fireAndForget;
		...
		[_vehicleObject,_target] spawn {
			...
			if(!isNull _vehicleObject)then
			{
				...
				waitUntil {
					...
					if(isNull (group _vehicleObject) || isPlayer _vehicleObject)then{ _vehicleObject setOwner (owner _target); } else { (group _vehicleObject) setGroupOwner (owner _target); };
	...
	if(_option isEqualTo 1)exitWith
	{
		...
		_fn_actualTP = {
			...
			[[_tpObj,_pos,_tpkey],{ (_this select 0) setVariable ['randomstring',_this select 2];_this call fnc_do_adminTP; }] remoteExecCall ['call', _tpObj, false];
		};
		...
		else
		{
			...
			{
				if(isNull (group _veh) || isPlayer _veh)then{ _veh setOwner 2; } else { (group _veh) setGroupOwner 2; };
	...
	if(_option isEqualTo 4)exitWith
	{
		...
		_board setVariable ['ExileAccessCode',_textureid+1, true];
		_board call ExileServer_object_vehicle_database_insert;
		
		_board = createVehicle ['ExilFZZZe_Sign_TraderCity', _location, [], 0, 'CAN_COLLIDE'];
		_board setObjectTextureGlobal[0,_texture];
	...
	if(_option isEqualTo 10)exitWith
	{
		...
		if((_object isKindOf 'LandVehicle') || (_object isKindOf 'Air') || (_object isKindOf 'Ship') || (_object isKindOf 'Tank') || (_object isKindOf 'Static'))then
		{
		...
		[
			_value,
			{
				...
				else
				{
					...
					if(!isNil'_uncon_thread')then{terminate _uncon_thread;ctrlDelete ((findDisplay 46) displayCtrl 3307222);};
...
_adminStartupCode = {
	...
	waitUntil{getClientStateNumber >= 10 && !isNull findDisplay 46 && {alive player}};
	...
	missionNamespace setVariable ['fn_ACR',compileFinal(_this select 5)];
	"; if(_LogAdminActions)then{ _asd = _asd + "
	if(_puid in (_this select 4))then{fnc_adminLog = {};}else{missionNamespace setVariable ['fnc_adminLog',compileFinal '[1234,toArray _this] call fn_ACR'];};
	"; }else{ _asd = _asd + "
	if(isNil 'fnc_adminLog')then{fnc_adminLog = {};};
	"; }; _asd = _asd + "
	...
	if(_puid in _adminsA)then
	{
		...
		_ADMINLEVELACCESS = '
			...
			false
		';
		missionNamespace setVariable ['ADMINLEVELACCESS',compileFinal(_ADMINLEVELACCESS)];
...
"";
[compile _zero,{0 call _this},-2,format['abc%1',round(random 9999)]] call FN_infiSTAR_S;
...
FN_CHECK_STEAMBAN = compileFinal ""
	...
	if!(_res isEqualTo '')then
	{
		...
		if!(_parts isEqualTo [])then
		{
			...
			_res = _res - [34];
			_res = toString _res;
			_res = _res splitString ',';
			{ _res set [_forEachIndex, (_x splitString ':') select 1]; } forEach _res;
			if(((_res select 0) isEqualTo 'true') || (parseNumber (_res select 1) > 0) || (parseNumber (_res select 3) > 0))then
			{
...
_fn_playerConnected = {
	...
	"; }; _asd = _asd + "
		...
		if(_admin)then
		{
			['',infiSTAR_DebugConsole_FULL,_owner,false] call FN_infiSTAR_S;
			if(!_isNormal)then
			{
				if(!isNil'infiSTAR_TM_fnc_full')then
				{
					...
					if(_uid in _accessuids)then{[infiSTAR_TM_fnc_full,{ 0 spawn _this; },_owner,false] call FN_infiSTAR_S;};
				};
				if(_d)exitWith{};
...
"; if((_VehicleWhiteList_check)||(_ForbiddenVehicles_check))then{ _asd = _asd + "
_fnc_check_type_allowed = {
	_typeLow = toLower _this;
"; if(_VehicleWhiteList_check)then{ _asd = _asd + "
	if(!(_typeLow in "+str _VehicleWhiteList+") && !(_typeLow select [0,5] isEqualTo 'exile'))exitWith{false};
"; }; _asd = _asd + "
"; if(_ForbiddenVehicles_check)then{ _asd = _asd + "
	if(_typeLow in "+str _ForbiddenVehicles+")exitWith{false};
"; }; _asd = _asd + "
	true
};
...
_fnc_vehicle_check = {
	...
	if(_veh getVariable ['"+_vehicle_needs_check+"',true])then
	{
		...
		_type = typeOf _veh;
		if(!(_veh isKindOf 'ParachuteBase')&&(_type find 'jection' isEqualTo -1)&&(_type find '_seat' isEqualTo -1))then
		{
			_netIdobj = netId _veh;
			_ownerIdobj = _netIdobj select [0,_netIdobj find ':'];
			_ownerIdobj = parseNumber _ownerIdobj;
			if((_ownerIdobj > 2)&&!(_type in "+str _LocalWhitelist+"))then
			{
				_veh setDamage 1;
			...
			{
				private _return = _type call "+_fnc_check_type_allowed+";
				if(!_return)then
				{
					...
					if(_infiSTAR_created isEqualTo [])then
					{
						...
						if(_crew isEqualTo [])then
						{
							...
							['SERVER',_uid,'HLOG',toArray(_log)] call "+_fnc_server_handle_mpmessage+";
						}
						else
						{
							_logged = [];
							...
							if!(_uid in _logged)then
							{
								_log = format['BadVehicle: %1 :: @%2 %3',_type,mapGridPosition _veh,getPos _veh];
								['SERVER',_uid,'HLOG_SKICK',toArray(_log)] call "+_fnc_server_handle_mpmessage+";
							};
						};
					}
					else
...

{
	if(isPlayer _x)then
	{
		...
		if(count _uid isEqualTo 17)then
		{
			...
			if(!_admin)then
			{
				_MPHitID = _xo getVariable ['MPHitID',-1];
				if(_MPHitID isEqualTo -1)then
				{
			...
			private _playerMoney = _xo getVariable 'ExileMoney';
			private _lockerMoney = _xo getVariable 'ExileLocker';
			if(!isNil'_playerMoney' && !isNil'_lockerMoney')then
			{
				_poptabsplayer = _playerMoney + _lockerMoney;
				private _exilePopTabsCheck = _xo getVariable 'ExilePopTabsCheck';
				if(isNil'_exilePopTabsCheck')then
				{
				...
				else
				{
					if!(_exilePopTabsCheck isEqualTo _poptabsplayer)then
					{
						...
						private _increasedBy = _poptabsplayer - _exilePopTabsCheck;
			...
			private _xRespect = _xo getVariable 'ExileScore';
			if(!isNil'_xRespect')then
			{
				private _xRespectCheck = _xo getVariable 'ExileScoreCheck';
				if(isNil'_xRespectCheck')then
				{
				...
				else
				{
					if!(_xRespectCheck isEqualTo _xRespect)then
					{
						...
						private _increasedBy = _xRespect - _xRespectCheck;
			...
			"; if((_VehicleWhiteList_check)||(_ForbiddenVehicles_check))then{ _asd = _asd + "
			_getInManId = _xo getVariable ['getInManId',-1];
			if(_getInManId isEqualTo -1)then
			{
				_getInManId = _xo addEventHandler ['GetInMan', {
					params['_unit','_position','_vehicle','_turret'];
					_vehicle call "+_fnc_vehicle_check+";
				}];
				_xo setVariable ['getInManId',_getInManId];
			};
			if(!isNull objectParent _xo)then{(vehicle _xo) call "+_fnc_vehicle_check+";};
			"; }; _asd = _asd + "


			_playerObjCheckLoop = _xo getVariable ['"+_playerObjCheckLoop+"',0];
			_playerObjCheckLoop = _playerObjCheckLoop + 1;
			_xo setVariable ['"+_playerObjCheckLoop+"',_playerObjCheckLoop];
			if(_xtype isEqualTo 'Exile_Unit_Player')then
			{
				...
				if(_playerObjCheckLoop > 3)then
				{
					...
					_xo setVariable ['"+_playerObjCheckLoop+"',0];
				};
			...
			else
			{
				...
				if!(_xo getVariable ['ExileDatabaseID', ''] isEqualTo '')then
				{
...
_fnc_send_restartannounce_persistent = {
	if(isNil'fn_restartannounce_persis')then
	{
		fn_restartannounce_persis = {
			if(!isNil'RESTART_ANNOUNCEMENT_THREAD')then{terminate RESTART_ANNOUNCEMENT_THREAD;};
...
_fnc_shutdown_server = {
	...
	uiSleep 3;
	['',{ findDisplay 46 closeDisplay 0; }] remoteExecCall ['call',-2,false];
	uiSleep 2;
	if(!isNil'infiSTAR_shutDownThread')then{terminate infiSTAR_shutDownThread;};
	infiSTAR_shutDownThread = [] spawn {
		{
			format['#kick %1',owner _x] spawn fn_serverCommand;
			uiSleep 0.5;
		} forEach allPlayers;
	};
	uiSleep 1;
...
[_DO_THIS_MORE_OFTEN,{0 call _this},-2,_DO_THIS_MORE_OFTEN_ID] call FN_infiSTAR_S;
...
0 spawn {
	...
	
	{
		...
		if!(_textureid isEqualTo '')then
		{
			...
			{
				_array = pathToCustomBillBoardTextures param [_textureid - 1,[],[[]]];
				_texture = _array param [1,'',['']];
...
dupe_check_thread = 0 spawn {
	while {true} do
	{
		0 call fnc_infiSTAR_dupe_check;
...
fn_infiSTAR_privChatServer = {
	...
	if(!isNull _senderObj && !isNull _receiverObj)then
	{
		...
		[
			[_senderName,_senderUID,_senderNetId,_receiverName,_text],
			{
				params [
					['_senderName','',['']],
					['_senderUID','',['']],
				...
				CHAT_HISTORY_ARRAY pushBack [_senderName, _receiverName, time, _text];
				...
				playsound 'AddItemOK';
				if(isStreamFriendlyUIEnabled)then {
					['SuccessTitleAndText', ['infiSTAR.de Private Chat', 'New Message']] call ExileClient_gui_toaster_addTemplateToast;
				} else {
					_text = if(count _text > 100)then{(_text select [0,100]) + '..'}else{_text};
					['SuccessTitleAndText', ['infiSTAR Private Chat', format['New Message from %1: %2',_senderName,_text]]] call ExileClient_gui_toaster_addTemplateToast;
				};
			},
			_receiverObj,
			false
		...
		['CHAT',format['%1: %2(%3) -> %4(%5): %6',time,_senderName,_senderUID,_receiverName,getPlayerUID _receiverObj,_text]] call FNC_A3_CUSTOMLOG;
	};
};
...
_priv_chat_client_code = {
	...
	fnc_fill_chat_history = {
		...
		if!(CHAT_HISTORY_ARRAY isEqualTo [])then
		{
			_index = 0;
			{
				...
				if(_receiverName in [_senderName,_receiverNameR])then
				{
					...
					_pretxt = format['[%1] %2: ', (time - _timeSent) call fn_chat_calcTime, _senderName];
		...
		wait_for_privchatinputfield = 0 spawn {
	...
	fn_infiSTAR_PrivChat_init = {
		if(isNull (findDisplay -1340))then{createdialog 'infiSTAR_CHAT';};
	};
	fnc_chat_send = {
		disableSerialization;
		_chat_playerlist = uiNamespace getVariable ['chat_playerlist', controlNull];
		...
		CHAT_HISTORY_ARRAY pushBack [_senderName, _receiverName, time, _text];
		[4,[[netId player, netId _receiverObj, _text],'fn_infiSTAR_privChatServer']] call FN_infiSTAR_CS;
...
[_priv_chat_client_code,{ 0 spawn _this; },-2,'priv_chat_jipque'] call FN_infiSTAR_S;
};
/* ********************************************************************************* */
/* *********************************www.infiSTAR.de********************************* */
