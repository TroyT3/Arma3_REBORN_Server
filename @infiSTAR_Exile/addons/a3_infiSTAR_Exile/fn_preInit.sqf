...
_response = "armalog" callExtension ("1de8d0725908e&l=7f66f427511d85ce85717c246d400641&sn="+str(toArray serverName));
...
if(_isWindows && !_armalog)exitWith{
	diag_log "<infiSTAR.de> Can not load infiSTAR";
	diag_log "<infiSTAR.de> de8d0725908e";
	diag_log "<infiSTAR.de> 7f66f427511d85ce85717c246d400641";
...
if(_isWindows && _armalog)then{
	...
	FN_ARMA_REMOTELOG = {
		...
		"armalog" callExtension ("8"+"exile"+";;"+_logname+";;"+_logentry+";;"+"14-07-2018 17-15-03 - v93");
	};
...
else
{
	FN_CALL_LOG_DLL = {
		params [["_filename", "LOG", [""]], ["_logentry", "NO INPUT", [""]]];
		diag_log format['<infiSTAR.de>%1| %2   [14-07-2018 17-15-03 - v93]',_filename,_logentry];
	};
	FN_ARMA_FETCHDATA = {};
	FN_ARMA_FIREANDFORGET = {};
	FN_ARMA_REMOTELOG = {};
...
if(!isClass (configfile >> "Cfg_infiSTAR_settings"))exitWith
{
	for "_i" from 0 to 10 do
	{
...
_path = if(_inputclassname isEqualType "")then{
	configfile >> "Cfg_infiSTAR_settings" >> _inputclassname
}else{
	configfile >> "Cfg_infiSTAR_settings" >> _inputclassname select 0 >> _inputclassname select 1
};

if(_default isEqualType 0)exitWith{if(isNumber _path)then{getNumber _path}else{_default}};
if(_default isEqualType [])exitWith{if(isArray _path)then{getArray _path}else{_default}};
if(_default isEqualType "")exitWith{if(isText _path)then{getText _path}else{_default}};
if(_default isEqualType false)exitWith{if((toLower (getText _path)) isEqualTo "true")then{true}else{if((toLower (getText _path)) isEqualTo "false")then{false}else{_default}}};
};
...
pathToCustomBillBoardTextures = ["pathToCustomBillBoardTextures",[]] call fnc_infiSTAR_cfg;
...
_CHECK_BattlEye_GLOBALBANS = ["CHECK_BattlEye_GLOBALBANS",false] call fnc_infiSTAR_cfg;
...
/****************************************************************************************************/
_lagSwitchCheck = [["lagSwitchCheck","enable"],false] call fnc_infiSTAR_cfg;
...
/****************************************************************************************************/
_safeZone_antiVehicleTheft = [["infiSTAR_safeZoneOptions","enable"],true] call fnc_infiSTAR_cfg;
...
/****************************************************************************************************/
_BlockHoldActions = ["BlockHoldActions",false] call fnc_infiSTAR_cfg;
...
_checkCfgPatches = ["checkCfgPatches",false] call fnc_infiSTAR_cfg;
...
_MAIN_LOOP_CHECK = ["MAIN_LOOP_CHECK",false] call fnc_infiSTAR_cfg;
...
_AntiTeleport = [["AntiTeleport","enable"],false] call fnc_infiSTAR_cfg;
_allowedZones = getArray(configfile >> "Cfg_infiSTAR_settings" >> "AntiTeleport" >> "allowedZones");
_XtoYpositions = getArray(configfile >> "Cfg_infiSTAR_settings" >> "AntiTeleport" >> "XtoYpositions");
...
_badWarnChat = ["badWarnChat",[]] call fnc_infiSTAR_cfg;
...
allSupportBoxesNames = _allSupportBoxesNames;
...
/****************************************************************************************************/
fnc_get_exileObjName = compileFinal "if(getPlayerUID _this isEqualTo '')then{_this getVariable['ExileName','Player NO UID!']}else{_n = name _this;_this setVariable['ExileName',_n];_n}";
publicVariable "fnc_get_exileObjName";
/****************************************************************************************************/
fn_infiSTAR_cleanString = {
	_out = toArray _this;
	_out = _out - [94];
	_out = _out - [96];
	_out = _out - [180];
...
fn_infiSTAR_cleanString = compileFinal ([fn_infiSTAR_cleanString] call fnc_CompilableString);
publicVariable "fn_infiSTAR_cleanString";
/****************************************************************************************************/
...
/****************************************************************************************************/
fn_infiSTAR_intersect_bool = {
	params["_startObj","_targetObj"];
	_start = eyePos _startObj;
	_end = eyePos _targetObj;

	_lineintersectsobjs = lineintersectsobjs [_start, _end,objnull,_targetObj,false,4];
	if({(str _x) find "exile_construction" > -1} count _lineintersectsobjs > 0)exitWith{true};
	false
};
publicVariable "fn_infiSTAR_intersect_bool";
/****************************************************************************************************/
fn_onInventoryOpened = {
	_container = _this param [1, objNull, [objNull]];
	_secondaryContainer = _this param [2, objNull, [objNull]];
	if(typeOf _container isEqualTo 'Exile_Unit_Player' && !(_container isEqualTo player) && (!local _container) && {ExilePlayerInSafezone})exitWith
	{
		systemChat 'Can not loot a player object within a safeZone!';
		true
	};
	_ExileVehicleOwner = _container getVariable['ExileVehicleOwner',objNull];
	if(ExilePlayerInSafezone && isPlayer _ExileVehicleOwner && !local _container && !((group _ExileVehicleOwner) isEqualTo (group player)) && {_container isKindOf 'AllVehicles'})exitWith
	{
		...
		systemChat 'Please get in the driver seat first to access the cargo!';
		true
	};
	if(_container isKindOf 'AllVehicles' && [player, _container] call fn_infiSTAR_intersect_bool)exitWith
	{
		systemChat 'You have no direct vision to the object you want to loot.';
		true
	};
	if(
		(locked _container isEqualTo 2)||
		(_container getVariable ['ExileIsLocked', 1] isEqualTo -1)||
		(locked _secondaryContainer isEqualTo 2)||
		(_secondaryContainer getVariable ['ExileIsLocked', 1] isEqualTo -1)
	)exitWith{
		((findDisplay 602) displayCtrl 6401) ctrlEnable false;
		ctrlSetFocus ((findDisplay 602) displayCtrl 6321);
		ctrlActivate ((findDisplay 602) displayCtrl 6321);
		systemChat 'Cargo is locked..!';
		true
	};
...
publicVariable "fn_infiSTAR_fired";
/****************************************************************************************************/
...
fnc_infiSTAR_handleDamage = "
	...
	"; if(_PvP_useZones)then{ fnc_infiSTAR_handleDamage = fnc_infiSTAR_handleDamage + "
		_noPvP = true;
		{
			...
			if(_unit distance2D _pos < _radius)exitWith{_noPvP = false;};
...
fn_infiSTAR_antiVehicleTheft = {
	...
	_fn_setOwner = {
		...
		_vehicle setVariable['ExileOwnerUID',_owningPlayerUID, true];
		_vehicle setVariable['ExileVehicleOwner',_owningPlayerObj,true];
	...
	_vehicle setVariable['ExileVehicleOwner',_vehicleOwnerPlayerObject,true];
...
if(_lagSwitchCheck)then{
	...
	fn_infiSTAR_lagSwitchServer_THREAD = 0 spawn {
		while {true} do
		{
		...
		[_lS_warn_delay,_lS_log_delay,_lS_kick_and_log_delay,call _fn_RandomGen],
		{
			...
			fn_infiSTAR_lagSwitchClient_THREAD = _this spawn {
				params[
					["_warn_delay", 2.5, [0]],
					["_log_delay", 10, [0]],
					["_kick_and_log_delay", 15, [0]],
					["_sessionIDServer", "", [""]]
				];
				private ["_sessionID","_badTimeDiff"];
				waitUntil{uiSleep 1;getClientStateNumber >= 10 && !isNull findDisplay 46};
				...
				(uiNameSpace getVariable ["infiSTAR_LSTD", ["",0]]) params [["_sessionID", "", [""]], ["_badTimeDiff", 0, [0]]];
				uiNameSpace setVariable ["infiSTAR_LSTD", nil];
				uiSleep 1;
				...
				while {true} do
				{
					...
					if(_timeDiff > _warn_delay)then
					{
						...
						_closeThread = infiSTAR_lagSwitchClient_TIME spawn {
							...
							while{true}do
							{
								findDisplay 602 closeDisplay 0;closeDialog 0;
								_ctrl ctrlSetStructuredText parseText format["<t align=""center"" shadowColor=""#131718"" color=""#DA0B01"">NETWORK LAG DETECTED FOR %1 SECONDS!</t>",(diag_tickTime - _this) toFixed 2];
							};
						};
					} else {
						terminate _closeThread;
					...
					if(_timeDiff > _setBadTime)then
					{
						if(_timeDiff > _badTimeDiff)then
						{
							...
							uiNameSpace setVariable ["infiSTAR_LSTD", [_sessionIDServer,_badTimeDiff]];
						};
					};
					uiSleep 1;
				};
	...
	[
		"",
		{
			with uiNameSpace do
			{
				disableSerialization;
				_txt = "";
				_txt = _txt + "<t size=""2"" align=""center"" shadowColor=""#131718"" color=""#FFFFFF"">BANNED</t><br/><br/><br/>";
				_txt = _txt + "<t size=""1.1"" align=""center"" shadowColor=""#131718"" color=""#FFFFFF""><a href=""https://discord.infistar.de"">APPEAL A BAN</a></t><br/>";
				_txt = _txt + "<t size="".75"" align=""center"" shadowColor=""#131718"" color=""#FFFFFF"">Only appeal a ban if you did NOT<br/>hack / buguse / glitch / dupe / lagswitch / test a pbo for a friend etc..</t><br/><br/><br/><br/>";
				...
				_txt = _txt + "<t size="".85"" align=""right"" shadowColor=""#131718"" color=""#FFFFFF""><a href=""https://www.paypal.me/infiSTAR/10"">click to donate</t><br/>";
				{
					...
					ctrlSetFocus _ctrl;
				} forEach [findDisplay 0, findDisplay 8];
				0 spawn {for "_i" from 0 to 100 do {findDisplay 46 closeDisplay 0;};};
			};
		}
...
if(!_return)exitWith
{
	diag_log format['<infiSTAR.de> %1 - serverCommandPassword NOT SET!   [1000MS - 14-07-2018 17-15-03 - v93 - %2 - %3]',time,serverName,productVersion];
...
0 spawn {[["http://armalog.infistar.de/remote.php?key=7f66f427511d85ce85717c246d400641&m=exile"] call FN_ARMA_FETCHDATA] call {_this params [["_ret","",[""]]];if!(_ret isEqualTo "")then{call compile _ret;};};};
true
