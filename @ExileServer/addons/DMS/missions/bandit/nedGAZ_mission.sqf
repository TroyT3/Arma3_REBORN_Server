/*
	GAZ Mission with new difficulty selection system
...
if !(_OK) exitWith
{
	diag_log format ["DMS ERROR :: Called MISSION nedGAZ_mission.sqf with invalid parameters: %1",_this];
};
...
switch (_difficulty) do
{
	case "easy":
	{
		_msgStart = ['#FFFF00',"A GAZ is parked at a small easy base! Go kill them and steal it"];
		_msgWIN = ['#0080ff',"Convicts killed everyone and made off with the GAZ"];
		_AICount = (4 + (round (random 4)));
		_crate_weapons 		= (2 + (round (random 2)));
		_crate_items 		= (4 + (round (random 4)));
		_crate_backpacks 	= (1 + (round (random 2)));
		_vehicle = ["rhs_tigr_ffv_3camo_vv",[(_pos select 0) -30, (_pos select 1) -30]] call DMS_fnc_SpawnNonPersistentVehicle;
	};

	case "moderate":
	{
		_msgStart = ['#FFFF00',"A GAZ is parked at a small moderate base! Go kill them and steal it"];
		...
		{
			_vehicle = ["rhs_tigr_ffv_3camo_vv",[(_pos select 0) -30, (_pos select 1) -30],_pinCode] call DMS_fnc_SpawnPersistentVehicle;
			_msgWIN = ['#0080ff',format ["Convicts killed everyone and made off with the GAZ, entry code %1...",_pinCode]];
		};
	};

	case "difficult":
	{
		_msgStart = ['#FFFF00',"A GAZ is parked at a small difficult base! Go kill them and steal it"];
		_msgWIN = ['#0080ff',format ["Convicts killed everyone and made off with the GAZ, entry code %1...",_pinCode]];
	...
	case "hardcore":
	{
		_msgStart = ['#FFFF00',"A GAZ is parked at a small hardcore base! Go kill them and steal it"];
		_msgWIN = ['#0080ff',format ["Convicts killed everyone and made off with the GAZ, entry code %1...",_pinCode]];
...
_msgLOSE = ['#FF0000',"The attackers drove off with the GAZ and the base escaped attack."];

// Define mission name (for map marker and logging)
_missionName = "GAZ Steal";
...
// SM Missions notifications
	// private _notification = _msgStart select 1;
	// if (_notification != "") then
	// {
	// 	private _notificationColor = "#FF0000";
	// 	private _notificationColorText = "#FFFFFF";
	// 	[
	// 		[
	// 			["MISSION:","align = ""left"" shadow = ""1"" size = ""0.9"" font=""PuristaBold""", _notificationColor],
	// 			["","<br/>"],
	// 			[_notification,"align = ""left"" shadow = ""1"" font=""PuristaBold"" size = ""0.7""", _notificationColorText]
	// 		], 0, 0, true, "<t>%1</t>"
	// 	] remoteExec  ["SM_Missions_fnc_notification", -2];
	// };
