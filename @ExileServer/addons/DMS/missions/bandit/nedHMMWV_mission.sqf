/*
	HMMWV Mission with new difficulty selection system
...
if !(_OK) exitWith
{
	diag_log format ["DMS ERROR :: Called MISSION nedHMMWV_mission.sqf with invalid parameters: %1",_this];
};
...
switch (_difficulty) do
{
	case "easy":
	{
		_msgWIN = ['#0080ff',"Convicts killed everyone and made off with the HMMWV"];
		_AICount = (4 + (round (random 4)));
		_crate_weapons 		= (2 + (round (random 2)));
		_crate_items 		= (4 + (round (random 4)));
		_crate_backpacks 	= (1 + (round (random 2)));
		_vehicle = ["Exile_Car_HMMWV_UNA_Desert",[(_pos select 0) -30, (_pos select 1) -30]] call DMS_fnc_SpawnNonPersistentVehicle;
	};
	...
	case "moderate":
	{
		...
		{
			_vehicle = ["Exile_Car_HMMWV_UNA_Desert",[(_pos select 0) -30, (_pos select 1) -30],_pinCode] call DMS_fnc_SpawnPersistentVehicle;
			_msgWIN = ['#0080ff',format ["Convicts killed everyone and made off with the HMMWV, entry code %1...",_pinCode]];
		};
	};
...
//_msgStart = ['#FFFF00',format["An HMMWV is parked at a small %1 base! Go kill them and steal it",_difficulty]];
_msgStart = ['#FFFF00',"An HMMWV is parked at a small base! Go kill them and steal it"];
...
_msgLOSE = ['#FF0000',"The attackers drove off in the HMMWV and the base escaped attack."];

// Define mission name (for map marker and logging)
_missionName = "HMMWV Steal";
