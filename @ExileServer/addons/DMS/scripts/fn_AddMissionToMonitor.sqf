...
try
{
	...
	if (DMS_MarkerText_ShowAICount) then
	{
		...
		private _targetMSG = [
			...
			"of the bastards remaining",
			"targets remaining",
			"targets left",
			"sumbitches left",
			"hostiles alive",
			"targets drawing breath",
			"jagoffs afield",
			"souls to be claimed",
			"enemies not yet dead",
			"bogeys in the area",
			"contacts still hot",
			"warm bodies remaining",
			"scumbags left",
			"fools not learning",
			"bad guys breathing",
			"threats unneutralized",
			"unreaped souls",
			"chances for glory"
		];
		_markerDot setMarkerText (format ["%1 (%2 %3 remaining)",markerText _markerDot,_unitCount,(selectRandom _targetMSG)]);
	};
	...
	private _markerDotname = _markers select 0;
	if (DMS_MissionToBSFevents isEqualTo true) then {
		BSF_Event_Positions set [_markerDotname,[_pos, time + 14400,"Somrthing's up", "DMS Mission"]];
