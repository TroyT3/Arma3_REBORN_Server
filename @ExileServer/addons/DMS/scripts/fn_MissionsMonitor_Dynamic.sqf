		...
		*/
		...
		if (_completionInfo call DMS_fnc_MissionSuccessState) then
		{
...

			{
				...
				{
					_x lock 0;
					_x setVariable ["ExileIsLocked",0, true]; // BSF Edit
					...
					_x addItemCargoGlobal ["BSF_Item_PinkSlip", 1];	// BSF
		...
		private _targetMSG = [
			...
			_dot setMarkerText (format ["%1 (%2 %3 remaining)",_text,count (_units call DMS_fnc_GetAllUnits),(selectRandom _targetMSG)]);
