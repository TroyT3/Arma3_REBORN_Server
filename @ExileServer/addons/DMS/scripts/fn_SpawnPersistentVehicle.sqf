...
try
{
	...
	_vehObj setVariable ["ExileAccessCode", _pinCode, true]; // BSF Edit
	_vehObj setVariable ["ExileOwnerUID", "DMS_PersistentVehicle",true];		// Don't change this unless you know what you're doing.
	// Deny access until specified to do so.
	_vehObj setVariable ["BSF_MarkerOn", 0, true];
	_vehObj setVariable ["BSF_TerritoryID", 0, true];
	_vehObj setVariable ["ExileIsLocked",-1, true]; // BSF Edit
