...
switch (toLower worldName) do
{
	...
	case "malden":
    {
        DMS_MapCenterPos    = [4935,6700];
        DMS_MapRadius       = 7000;
    };   
	case "enoch":
    {
        DMS_MapCenterPos    = [6350,6350];
        DMS_MapRadius       = 7000;
    };   
...

{
	...
	[
		...
		compileFinal preprocessFileLineNumbers (format ["\x\addons\a3_dms\missions\bandit\%1.sqf",_x]) 
	];
...
if (DMS_StaticMission) then
{
...

	{
		...
		[
			...
			compileFinal preprocessFileLineNumbers (format ["\x\addons\a3_dms\missions\static\%1.sqf",_x])
		];
