...
if (_currentAIcount < Event_RoamingAI_MaxAllowedAI) then
{
	...
	{	
		...
		if (EventTownInvasionToBSFevents isEqualTo true) then {
			BSF_Event_Positions set [_marker,[position _holder, time + Event_TownInvasion_DespawnTime, "Town Invasion"]];
		...
		format ["[REBORN EVENT: Rioters] Rioters have spawned @ %1",_position] call ExileServer_util_log;
	};
};	