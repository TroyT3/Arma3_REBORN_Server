...
if ((Event_HeliCrash_Count < Event_HeliCrash_MaxEvents) && (random 1 > 0.3)) then
{
	...
	*/
	...
	_holder = createVehicle ["Exile_Container_SupplyBox", _landPos, [], 10, "CAN_COLLIDE"];
	...
	if (EventHeliCrashToBSFevents isEqualTo true) then {
		_marker = createMarker [ format["HeliCrash%1", diag_tickTime], _landPos];
		BSF_Event_Positions set [_marker,[position _crash, time + Event_HeliCrash_MarkerDuration, "Heli Crash (new)"]];
	...
	format ["[REBORN EVENT: HeliCrash] Spawned a helicrash at -- %1 | Current HELICRASH Count -- %2", _landPos,Event_HeliCrash_Count] call ExileServer_util_log;
}
else
{	
	format ["[REBORN EVENT: HeliCrash] Crash did not spawn -- Current crash count:%1 | Chance was not succesful",Event_HeliCrash_Count] call ExileServer_util_log;
};	