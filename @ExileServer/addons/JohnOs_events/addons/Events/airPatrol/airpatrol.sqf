...
"[REBORN EVENT: AirPatrol] Ambient air patrol initalized" call ExileServer_util_log;
...
if !(_debug) then
{    
    _startPos = selectRandom _rescueStartPos;
...
{    
    ...
    "[REBORN EVENT: AirPatrol] Ambient air patrol event started" call ExileServer_util_log;
    ...
    if (intercept) then
    {
        "[REBORN EVENT: AirPatrol] Interceptor aircraft has been despatched" call ExileServer_util_log;
        ...
        _interceptorStartPos = selectRandom _rescueStartPos;
//        _interceptorStartPos = _rescueStartPos call BIS_fnc_selectRandom;
    ...
    { 
        "[REBORN EVENT: AirPatrol] Crash sequence initiated" call ExileServer_util_log;
        ...
        */
        ...
        // BSF_Event_Positions pushBack [position _crash, time + 14400,"Crash site", "JohnO Air Patrol"];
        if (EventAirPatrolToBSFevents isEqualTo true) then {
            _marker = createMarker [ format["AirPatrol%1", diag_tickTime], _landPos];
            BSF_Event_Positions set [_marker,[position _crash, time + 14400,"Crash site", "Air Patrol"]];
        ...
        if (intercept) then
        {
            ...
            "[REBORN EVENT: AirPatrol] Remaining offensive waypoints for interceptor deleted" call ExileServer_util_log;

            _interceptorExitPos = selectRandom _rescueStartPos;
//            _interceptorExitPos = _rescueStartPos call BIS_fnc_selectRandom;
            ...
            "[REBORN EVENT: AirPatrol] Interceptor retreat order given" call ExileServer_util_log;
        };
        ...
        "[REBORN EVENT: AirPatrol] Rescue QRF Helo despatched" call ExileServer_util_log;
...
//            _weapon = _lootWeapons call BIS_fnc_selectRandom;
        ...
        {
            /*
            _items = selectRandom _lootItems;
 //           _items = _lootItems call BIS_fnc_selectRandom;
       ...
       "[REBORN EVENT: AirPatrol] Rescue QRF Helo reached destination" call ExileServer_util_log;
...

        {
            ...
            _curWeapon = selectRandom _lootWeapons;
 //           _curWeapon = _lootWeapons call BIS_fnc_selectRandom;
        ...
        */
...

        {
			...
			_x setVariable ["BSF_SpawnedBy","Reborn",true]; // BSF
    ...
    {
       "[REBORN EVENT: AirPatrol] Aircraft crashed in water -- Terminating script" call ExileServer_util_log;
        ...
        if (intercept) then
        {
            ...
            "[REBORN EVENT: AirPatrol] Remaining offensive interceptor waypoints deleted" call ExileServer_util_log;
...
//            _interceptorExitPos = _rescueStartPos call BIS_fnc_selectRandom;
           ...
           "[REBORN EVENT: AirPatrol] Interceptor aircraft dismiss order given" call ExileServer_util_log;
        };
    };
...
{
    "[REBORN EVENT: AirPatrol] Script random chance failed -- Re trying event" call ExileServer_util_log;
