// JohnO_fnc_supplyDropObject_BSF.sqf
...
if (random 1 > 0.3) then
{
	...
	{
		format ["[REBORN EVENT: SupplyDrop] -- Supply spawned to close too trader zone OR debug island @ %1. Cancelling event",_randomPosition] call ExileServer_util_log;
	};
	...
	{	
		...
		*/
		_drop addItemCargoGlobal ["BSF_Item_PinkSlip", 1];	// BSF
		...
		"[REBORN EVENT: SupplyDrop] -- Supply drop event has spawned its cargo and populated loot" call ExileServer_util_log;
		...
		if (EventSupplyDropToBSFevents isEqualTo true) then {
			BSF_Event_Positions set [ _marker,[position _drop, time + Event_SupplyDrop_MarkerDuration, "Supply Drop"]];
			publicVariable "BSF_Event_Positions";
		};

		format ["[REBORN EVENT: SupplyDrop] Spawned supply drop event @ %1",_randomPosition] call ExileServer_util_log;
		Event_SupplyDrop_monitorCount = Event_SupplyDrop_monitorCount + 1;
	}
	else
	{
		"[REBORN EVENT: SupplyDrop] Did not spawn a supply drop vehicle - Vehicle selected max count on server reached" call ExileServer_util_log;	
	};	
}
else
{
	
	"[REBORN EVENT: SupplyDrop] Supply drop did not spawn | Chance was not succesful" call ExileServer_util_log;
};	
