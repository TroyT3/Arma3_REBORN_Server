...
// Begin Grasscutter objects code
		_cutterClass = (_data select 1);
		_case = {};
		if (_cutterClass == "BSF_GrassCutter_20") then {_case = 1};  
		if (_cutterClass == "BSF_GrassCutter_30") then {_case = 2}; 
		if (_cutterClass == "BSF_GrassCutter_40") then {_case = 3}; 
		...
		switch (_case) do {
			...
			case 3: 	{_terrainobjects = nearestTerrainObjects [_position, ["TREE", "SMALL TREE", "BUSH"], 40]; {hideObjectGlobal _x} foreach _terrainobjects;};
			};
// End Grasscutter objects code
