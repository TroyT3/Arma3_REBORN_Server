...
for "_i" from 1 to SC_numberofHeliCrashes do
{
	...
	_heli_marker = createMarker [ format ["SC_helicrash_marker_%1", _i], _positionOfBox];
	...
	if (SC_HeliCrashMarkers) then 
		...
		{_heli_marker setMarkerAlpha 0;};
	
	if (SC_HeliCrashesToBSFevents isEqualTo true) then {
		BSF_Event_Positions set [_heli_marker,[position _vehHeli, time + 14400, "Crash site", "Occupation Heli Crash"]];
