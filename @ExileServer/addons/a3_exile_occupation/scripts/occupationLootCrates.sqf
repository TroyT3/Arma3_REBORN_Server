...
[] spawn {
    waitUntil {
        !(isNil "BSF_Event_Positions") && {typeName BSF_Event_Positions == "HASHMAP"}
    };
};
...
for "_i" from 1 to SC_numberofLootCrates do
{
	...
	while{!_validspot} do 
	{
		...
		_nearOtherCrate = (nearestObjects [_position,["C_IDAP_supplyCrate_F"],500]) select 0;	
		_nearOtherCrate = (nearestObjects [_position,["BSF_SupplyBox_Orange"],500]) select 0;	
		_nearOtherCrate = (nearestObjects [_position,["BSF_SupplyBox_Green"],500]) select 0;	
		if (!isNil "_nearOtherCrate") then { _validspot = false; };	

	};	
	
	// private ["_event_marker"];
	...
	// 	_event_marker setMarkerBrush "Vertical";	//Added by Troy
	// 	_event_marker setMarkerSize [(1), (1)];		//Added by Troy
	// }
	// else
	// {
	// 	_event_marker = format ["SC_helicrash_marker_%1", _i];
	// };
	...
	if (SC_occupyLootCratesMarkers) then
		...
		{_event_marker setMarkerAlpha 0;};

	if (SC_LootCratesToBSFevents isEqualTo true) then {
		BSF_Event_Positions set [_event_marker,[_position, time + 14400, "Loot Crate", "Occupation Loot Crate"]];
	...
	if (SC_SpawnLootCrateGuards) then
	{
			...
			if(_AICount > 0) then
			{
				...
				{	
					...
					_unit setVariable ["BSF_SpawnedBy","Occupation",true];  // BSF
...
//	_box = "C_IDAP_supplyCrate_F" createvehicle _position;
	_box = "BSF_SupplyBox_Orange" createvehicle _position;
