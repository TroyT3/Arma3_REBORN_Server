...
*/
private ["_option","_display","_playerUID"];
        ...
        switch (_location) do
        {
                ...
                forEach [21020,21021,21022,21023,21031,21032];
                {
                    ...
                    if ((_x select 1) isEqualTo 1) then
                    {
                        ...
                        if (!(_configName isEqualTo "CfgVehicles") || (_configName isEqualTo "CfgVehicles" && _type isEqualTo 3)) then
                        {
                            ...
                            if (_sellersUID isEqualTo _playerUID) then
            ...
            case 1: // Vehicles?
            {
                private ["_text","_ClassName","_listingID","_price","_configName","_name","_index","_fuel","_health","_sellerUID","_type"];
                {
                    ...
                    _listingFee = _vehicleCost * 0.05;
                    // _rekeyCost = _vehicleCost * (getNumber (missionConfigFile >> "CfgTrading" >> "rekeyPriceFactor"));
                    _price = str(parseNumber(_x select 3) + _listingFee);
                    // _price = str(parseNumber(_x select 3) + _rekeyCost);
                    diag_log format["MARXET _ClassName %1, %2",_ClassName, typeName _ClassName];
                    diag_log format["MARXET _vehicleCost %1, %2",_vehicleCost, typeName _vehicleCost];
                    diag_log format["MARXET _listingFee %1, %2",_listingFee, typeName _listingFee];
                    diag_log format["MARXET _price %1, %2",_price, typeName _price];
                    ...
                    if ((_x select 1) isEqualTo 1) then
                    {
                        ...
                        if (_configName isEqualTo "CfgVehicles" && _type != 3) then
                        {
                            ...
                            // if (_sellersUID isEqualTo (getPlayerUID player)) then
                            {
                                _price = str(_listingFee);
                                // _price = str(_rekeyCost);
                            };
                     diag_log format["MARXET _price 2 %1, %2",_price, typeName _price];
                            ...
                            _text = format["%1:%2:%3:%4:%5:%6:%7",_name,_price,_listingID,_health,_fuel,_sellersUID,_listingFee];
...
////////////////////////////////
                _AllCloseVehicles = nearestObjects [player, ["LandVehicle", "Air", "Ship"], 50];
               _nearVehicles = [];
                {
        ...
        switch (_this select 1) do
        {
            case 0:
            {
                ...
                if !(_dataString isEqualTo "") then
                {
                    ...
                    if ( _sellerUID isEqualTo _playerUID) then
