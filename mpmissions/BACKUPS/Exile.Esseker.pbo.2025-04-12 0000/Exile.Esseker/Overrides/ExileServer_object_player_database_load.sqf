...
_player setVariable ["BSF_Admin", (_data select 47), true];
_player setVariable ["BSF_Donor", (_data select 48), true];
...
///////////// BSF Changelog Ackowledgment Check
private _lastGreetingDate = _data select 49;
while {(count _lastGreetingDate) > 5} do {
	_lastGreetingDate deleteAt (count _lastGreetingDate - 1);
};

private _lastGreetingNum = dateToNumber (_data select 49);
private _releaseNum = dateToNumber BSF_Changelog_ReleaseDate;

if (_lastGreetingNum < _releaseNum) then {
	format ["_lastGreetingNum < _releaseNum TRUE"] call BSF_Server_Util_Debug;
	private _playerObject = _sessionID call ExileServer_system_session_getPlayerObject;
	private _clientID = owner _playerObject;
	BSF_Show_Changelog = true;
	_clientID publicVariableClient "BSF_Show_Changelog";
}
else
{
	BSF_Show_Changelog = false;
	_clientID publicVariableClient "BSF_Show_Changelog";
	format ["_lastGreetingNum < _releaseNum FALSE"] call BSF_Server_Util_Debug;
};
///////////// END BSF Changelog Ackowledgment Check
...
// PVP Check
_player setVariable ["BSF_PVPon", false, true];

// Load player vehicle info

format ["###########################################"] call BSF_Server_Util_Debug;
format ["START PLAYER VEHICLE LOADING FOR %1", _name] call BSF_Server_Util_Log;
_playerVehicles = [_playerUID, _clanID] call BSF_Server_AssetManagement_getPlayerVehicles;
[_sessionID, "PlayerVehiclesCreateResponse", [_playerVehicles]] call ExileServer_system_network_send_to;
format ["DB_Load _player %1 - %2", _player, typeName _player] call BSF_Server_Util_Debug;
format ["DB_Load netId _player %1", netId _player] call BSF_Server_Util_Debug;
format ["DB_Load _sessionID %1 - %2", _sessionID,  typeName _sessionID] call BSF_Server_Util_Debug;
format ["DB_Load _playerVehicles %1 - %2", _playerVehicles,  typeName _playerVehicles] call BSF_Server_Util_Debug;
format ["END PLAYER VEHICLE LOADING FOR %1", _name] call BSF_Server_Util_Debug;
format ["###########################################"] call BSF_Server_Util_Debug;
// END Load player vehicle info 
...
// END RZ Infection Persistence Block

// BSF Reborn EVR load
_EVRresistanceDB = format["getEVR_Res:%1", _player getVariable["ExileOwnerUID",""]] call ExileServer_system_database_query_selectSingle;
_EVRresistance = _EVRresistanceDB select 0;
_player setVariable ["EVRresistance", _EVRresistance, true];
// End BSF Reborn EVR load
