...
_BSFdonorStatus = player getVariable ["BSF_Donor","No"];
_BSFadminStatus = player getVariable ["BSF_Admin","No"];

if ( getNumber(_recipeConfig >> "BSF_Donor") == 1 ) then
{
	switch (_BSFdonorStatus) do
		{
		case "Active": {
				["SuccessTitleAndText",[format["THANKS!"], format ["Thank you for your support!"]]] call ExileClient_gui_toaster_addTemplateToast;
				};
		case "No": {
				["ErrorTitleAndText",[format["<t size='27'>CAN NOT CONSTRUCT!</t>"], format ["<t size='23'>This recipe can only be constructed by active BSF donors.  Please consider helping us out.</t>"]]] call ExileClient_gui_toaster_addTemplateToast;
				_metSideConditions = false;
				};
		case "Cancelled": {
				["ErrorTitleAndText",[format["<t size='27'>CAN NOT CONSTRUCT!</t>"], format ["<t size='23'>This recipe can only be constructed by active BSF donors.  Please consider renewing your subscription.</t>"]]] call ExileClient_gui_toaster_addTemplateToast;
				_metSideConditions = false;
				};
		default {
				["ErrorTitleAndText",[format["<t size='27'>CAN NOT CONSTRUCT!</t>"], format ["<t size='23'>This recipe can only be constructed by active BSF donors.  Please consider helping us out.</t>"]]] call ExileClient_gui_toaster_addTemplateToast;
...
//		["ErrorTitleAndText", ["Failed to craft!", "You do not have enough inventory space left!"]] call ExileClient_gui_toaster_addTemplateToast;
		["ErrorTitleAndText", ["Failed to craft!", "You may not have enough inventory space left!"]] call ExileClient_gui_toaster_addTemplateToast;
	};
};
true