	...
	{
		{
			...
			if (_showItem) then
			{
				...
				_items pushBackUnique [_itemClassName, getNumber(_tradingConfig >> format["Level%1", _quality]), _quality];
			};
		} forEach (getArray(missionConfigFile >> "CfgTraderCategories" >> _categoryClass >> "items"));
	};
...
{
	diag_log format["STOKESMAGEE 4: %1", _x];
	_x params ["_itemClassName","_requiredRespect","_quality"];
...
} forEach ([_items, [], {_x select 1}, "ASCEND", {true}] call BIS_fnc_sortBy);
