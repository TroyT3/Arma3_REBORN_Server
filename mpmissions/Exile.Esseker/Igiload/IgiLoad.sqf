...
if (isnil "IL_Variables") then
{
    ...
    [
		...
		"C_Offroad_01_bluestorm_F",
		"C_Offroad_01_default_F",
		"C_Offroad_01_stripped_F",
		"C_Offroad_01_luxe_F",
        "B_G_Offroad_01_F",
        "O_G_Offroad_01_F",
        "I_G_Offroad_01_F",
    ...
    IL_Supported_Vehicles_HUNTER =	// Vehicles that can have cargo loaded into them
    [
       "Exile_Car_Hunter",
       "B_MRAP_01_hmg_F",
       "B_T_MRAP_01_F",
    ...
    IL_Supported_Hunter =	// Vehicles that can be loaded as cargo
    [
        "Exile_Car_Hunter",
        "B_MRAP_01_hmg_F",
        "B_MRAP_01_gmg_F",
    ...
    [
        ...
        "Exile_Container_SupplyBox",
        //"Land_GarbageContainer_closed_F", // EBM - Used for Loot & Death ExileZ mission
        "Land_Icebox_F", // EBM
    ...
    [
        ...
        //"Land_Pod_Heli_Transport_04_fuel_black_F", //to spawn on map
...
// NOTE       "load_offset" //how far back the cargo loads in

{
    ...
    IL_Init_Veh =
    {
        ...
        if (_obj_type in IL_Supported_Vehicles_HEMTT) then
        {
            ...
            {
                if ((isNil {_obj getVariable "zload"}) || (_force)) then {_obj setVariable["zload", -0.5, true];};
                if ((isNil {_obj getVariable "load_offset"}) || (_force)) then {_obj setVariable["load_offset", 0, true];}; //how far back the cargo loads in
           };
        };
        ...
        if (_obj_type in IL_Supported_Vehicles_HUNTER) then	// Can have cargo loaded into it (BSF)
        {
            if ((isNil {_obj getVariable "box_num"}) || (_force)) then {_obj setVariable["box_num", 0, true];};
 ...
 //         if ((isNil {_obj getVariable "zload"}) || (_force)) then {_obj setVariable["zload", -1, true];};
            if ((isNil {_obj getVariable "zload"}) || (_force)) then {_obj setVariable["zload", -1.15, true];};				// Where does the cargo sit vertically realative to top? of vehicle
            if ((isNil {_obj getVariable "load_offset"}) || (_force)) then {_obj setVariable["load_offset", 3.2, true];};	// Where does the cargo sit horizontally realative to front? of vehicle
        };
    ...
    IL_Init_Box =
    {
        ...
        if (_obj_type in IL_Supported_Hunter) then		// How much space does it take up as cargo in another vehicle (BSF)
        {
            _obj setVariable["slots", 7, true];
    ...
    IL_Move_Attach=
    {
	 ...
	 // [_v, _x, [_x_cargo_offset,_counter + 0.25 - _cargo_offset,_zload], [_x_cargo_offset,-4.5,_zload], 1, _turn] call IL_Move_Attach;
        ...
        _veh = _this select 0;
        _obj = _this select 1;
        _from = _this select 2;		// [_x_cargo_offset,_counter + 0.25 - _cargo_offset,_zload] 		// From Hunter  (BSF)
        _to = _this select 3;		// [_x_cargo_offset,-4.5,_zload]									// From Hunter  (BSF)
    ...
    IL_Do_Load =
    {
        ...
        if ((_obj_type in IL_Supported_Vehicles_HUNTER) && (_doors == "B")) then 	// Can have cargo loaded into it (BSF)
        {
            _sdist = IL_SDistL;
            _spoint = _v modelToWorld [0,-4.5,-1.6]; // Where cargo needs to be sitting in order to load?
            _box_num = _v getVariable "box_num";
            _slot_num = _v getVariable "slots_num";
        };
                ...
                if (_x getVariable "doors" == _doors) then
                {
                    ...
                    if (((_x getVariable "attachedTruck") == _v) && ((_x getVariable "attachedPos") == (_counter)) && (_counter < 0) && !_done) then
                    {
                        ...
                        if ((_obj_type in IL_Supported_Vehicles_MOHAWK)  && (_doors == "B")) then
                        {
                            ...
                            [_v, _x, [_x_cargo_offset,-4.5,-0.75 + _zload], [_x_cargo_offset,-8,-0.75 + _zload], 1, _turn] call IL_Move_Attach;
                            if ((_para) && (_obj_type in IL_Para_Drop_Vehicles)) then
                            {
                        ...
                        if ((_obj_type in IL_Supported_Vehicles_HURON)  && (_doors == "B")) then
                        {
                            ...
                            {
                                // _last_attach_pos = [_x_cargo_offset,-10,-0.75 + _zload];
                                _last_attach_pos = [_x_cargo_offset,-15,-0.75 + _zload];
                            };
                        };
                        if ((_obj_type in IL_Supported_Vehicles_HUNTER) && (_doors == "B")) then 	// Can have cargo loaded into it (BSF)
                        {
                         // [_v, _x, [_x_cargo_offset,_counter + 0.25 - _cargo_offset,_zload], [_x_cargo_offset,-4.5,_zload], 1, _turn] call IL_Move_Attach;
                            [_v, _x, [_x_cargo_offset,_counter + 0.25 - _cargo_offset,_zload], [_x_cargo_offset,-5.6,_zload], 1, _turn] call IL_Move_Attach;
                        };
                        if ((_obj_type in IL_Supported_Vehicles_WOLFHOUND) && (_doors == "B")) then
                        {
                            [_v, _x, [_x_cargo_offset,_counter + 0.25 - _cargo_offset,_zload], [_x_cargo_offset,-4.5,_zload], 1, _turn] call IL_Move_Attach;
...
if (_obj_main_type in IL_Supported_Vehicles_URAL_COVERED) then
{
    ...
    _obj_main addAction [
    "<img image='IgiLoad\images\load.paa' /><t color=""#ffff00"">Load cargo on truck</t>",
    {
    ...
    _obj_main addAction [
    "<img image='IgiLoad\images\load.paa' /><t color=""#ffff00"">Load vehicle on truck</t>",
    {
    ...
    _obj_main addAction [
    "<img image='IgiLoad\images\unload.paa' /><t color=""#ffff00"">Unload cargo from truck</t>",
    {
        [_this select 0] call IL_Do_Unload;
    },[],IL_Action_O_Priority,false,true,"",
    "(_target getVariable 'box_num' < 0) && ((IL_Can_Inside && (driver _target == _this)) || (((_this in (nearestObjects[ _target modelToWorld [0,-4.5,0], [], IL_SDistL + IL_SDistL_Heli_offset])) && (_target getVariable 'can_outside')))) && (_target getVariable 'can_load') && (abs(speed _target) <= IL_LU_Speed)"
...
if (_obj_main_type in IL_Supported_Vehicles_HUNTER) then
{
    ...
    "(_target getVariable 'box_num' < 0) && ((IL_Can_Inside && (driver _target == _this)) || (((_this in (nearestObjects[ _target modelToWorld [0,-4.5,0], [], IL_SDistL + IL_SDistL_Heli_offset])) && (_target getVariable 'can_outside')))) && (_target getVariable 'can_load') && (abs(speed _target) <= IL_LU_Speed)"	// (BSF)
    ];
