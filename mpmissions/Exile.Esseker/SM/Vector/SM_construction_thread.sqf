...
_position = ctrlPosition (_display displayCtrl 1002);
_position = ctrlPosition (_display displayCtrl 1002);
...
_h = _h + 0.05;
(_display displayCtrl 1002) ctrlSetPosition [_x,_y,_w,_h];
...
SM_memForBack = 0;
SM_memLeftRight = 0;
_vector = [SM_memForBack,SM_memLeftRight,_rotation] call SM_CalculateVector;
	...
	if !(ExileClientConstructionLock) then
	{
		...
		switch (ExileClientConstructionMode) do
		{
			case 1:
			{
				...
				_vector = [SM_memForBack,SM_memLeftRight,_rotation] call SM_CalculateVector;
				_vectorUp = _vector select 0;
				_vectorDirection = _vector select 1;
			};
			case 2:
			{
				_position = ASLtoATL (AGLtoASL (player modelToWorld ExileClientConstructionOffset));
		...
		ExileClientConstructionObject setVectorDirAndUp [_vectorUp,_vectorDirection];
