...
try
{
	...
	// private _vehicleInfo = format["confirmVehicleOwnership:%1", _nickname] call ExileServer_system_database_query_selectSingle;
	...
	_vehID = _vehicleInfo select 0;
	_newVehObject = _vehID call ExileServer_object_vehicle_database_load; // AssetManagement
	...
	_newTerritoryID = "";
	_xData = [];
	diag_log format["retrieveVehicleRequest _vehID %1, %2",_vehID, typeName _vehID];

	[_sessionID, "PlayerVehiclesUpdateResponse", [["fetch", _vehID, _newTerritoryID, _newVehObject, _xData]]] call ExileServer_system_network_send_to;
