...
if (diag_tickTime - ExileClientPlayerLastHpRegenerationAt >= 60) then
{
	if (_hunger >= ExileClientHungerRegen) then
	{
		...
		{
			if (!(ExileClientPlayerIsInCombat) && !(player getVariable ["BSF_Reborn_Infected",false])) then
			{
				...
				{	
					["<t color='#00cc44' font='PuristaBold' size ='.6'>My wounds are healing</t>",(safezoneX + safezoneW) - 900 * pixelW - 60 * pixelW,(safezoneY + safezoneH) - 128 * pixelH - 60 * pixelH,5,1,0,789] spawn BIS_fnc_dynamicText;
				};	
