...
{
	// Check if the menu is not active, and there is not another dialog open.
	if (!SM_DynMenu_ActionKeyPressed && !SM_DynMenu_ActionInUse && !dialog) then
	{
		SM_DynMenu_ActionKeyPressed = true;
		[] spawn SM_DynMenu_Draw;
	};
	true
};
/*
...
// Dual Arms: Use User6 key to sling/swap primary without CBA
if (_keyCode in (actionKeys "User6")) exitWith
{
...
{ 
	...
	case 0x02: 	{ _stopPropagation = true; };
	case 0x03: 	{ _stopPropagation = true; };
	case 0x04: 	{ _stopPropagation = true; };
	case 0x05: 	{ _stopPropagation = true; };
	case 0xD2:	{ _stopPropagation = true; };
	case 0x10:
	{
