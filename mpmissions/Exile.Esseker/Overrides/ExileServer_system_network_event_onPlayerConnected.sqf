...
if !(_uid in ["", "__SERVER__", "__HEADLESS__"]) then
{
	...
	if (_isKnownAccount) then
	{
		...
		// Reborn
        _hasRebornEntry = format["hasRebornEntry:%1",_uid] call ExileServer_system_database_query_selectSingleField;
        if !(_hasRebornEntry) then
        {
            format["createRebornEntry:%1",_uid] call ExileServer_system_database_query_fireAndForget;
        };
		// END Reborn
	}
	else
