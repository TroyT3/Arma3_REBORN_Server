/**
 * ExileServer_object_vehicle_network_storeVehicleRequest
 *
 * Exile Mod
 * www.exilemod.com
 * © 2015 Exile Mod Team
 *
 * This work is licensed under the Creative Commons Attribution-NonCommercial-NoDerivatives 4.0 International License. 
 * To view a copy of this license, visit http://creativecommons.org/licenses/by-nc-nd/4.0/.
 */
params["_sessionID", "_parameters"];
private _netID = _parameters select 0;
private _nickname = _parameters select 1;
private _playerObject = _sessionID call ExileServer_system_session_getPlayerObject;
private _vehicleObject = objectFromNetID(_netID);
try
{
	if (getNumber(missionConfigFile >> "CfgVirtualGarage" >> "enableVirtualGarage") isEqualTo 0) then 
	{
		throw "Virtual Garage is disabled on this server";
	};
	if (isNull _playerObject) then 
	{
		throw "Player is null";
	};
	if (_playerObject getVariable ["ExileMutex",false]) then
	{
		throw "Player is Mutex";
	};
	_playerObject setVariable ["ExileMutex", true];
	if (isNull _vehicleObject) then 
	{
		throw "Vehicle is null";
	};
	if !(alive _vehicleObject) then 
	{
		throw "Vehicle has been destroyed";	
	};
	if (_vehicleObject getVariable ["ExileMutex",false]) then
	{
		throw "Vehicle is in the process of being stored";
	};
	_vehicleObject setVariable ["ExileMutex", true];
	if !(_vehicleObject getVariable ["ExileIsPersistent", false]) then 
	{
		throw "You cannot store non-persistent vehicles in the garage!";
	};
	private _alphabet = getText (missionConfigFile >> "CfgClans" >> "clanNameAlphabet");
	private _forbiddenCharacter = [_nickname, _alphabet] call ExileClient_util_string_containsForbiddenCharacter;
	if !(_forbiddenCharacter isEqualTo -1) then
	{
		throw "Invalid vehicle nickname";
	};
	private _flagObject = _playerObject call ExileClient_util_world_getTerritoryAtPosition;
	private _storedVehicles = _flagObject getVariable ["ExileTerritoryStoredVehicles", []];
	private _level = _flagObject getVariable ["ExileTerritoryLevel", 1];
	private _maxNumberOfVehicles = getArray(missionConfigFile >> "CfgVirtualGarage" >> "numberOfVehicles") select ((_level - 1) max 0);
	if (_maxNumberOfVehicles isEqualTo -1) then 
	{
		throw "Your territory isn't allowed to store any vehicles.<br />Upgrade your territory to gain access to Virtual Garage";
	};
	if (count(_storedVehicles) >= _maxNumberOfVehicles) then 
	{
		throw "You cannot store any more vehicles in your territory.<br />Upgrade your territory to store more vehicles!";
	};
	if ((getPosATL _vehicleObject) distance (getPosATL _flagObject) > (_flagObject getVariable ["ExileTerritorySize", 0])) then 
	{
		throw "Vehicle must be in your territory";
	};
	if (getNumber(missionConfigFile >> "CfgVirtualGarage" >> "clearInventoryOnStore") isEqualTo 1) then 
	{		
		private _vehiclePosition = getPosATL _vehicleObject;
		private _items = _vehicleObject call ExileServer_util_getItemCargo;
		private _magazines = magazinesAmmoCargo _vehicleObject;
		private _weapons = weaponsItemsCargo _vehicleObject;
		private _containers =_vehicleObject call ExileServer_util_getObjectContainerCargo;
		private _popTabs = _vehicleObject getVariable ["ExileMoney", 0];
		private _groundHolder = createVehicle ["GroundWeaponHolder",_vehiclePosition, [], 0, "CAN_COLLIDE"];
		_groundHolder setPosATL _vehiclePosition;
		if (_popTabs > 0 ) then 
		{
			private _popTabsObject = createVehicle ["Exile_PopTabs", _vehiclePosition, [], 0, "CAN_COLLIDE"];	
			_popTabsObject setVariable ["ExileMoney", _popTabs, true];
			_vehicleObject setVariable ["ExileMoney", 0];
		};
		[_groundHolder,_items] call ExileServer_util_fill_fillItems;
		[_groundHolder,_magazines] call ExileServer_util_fill_fillMagazines;
		[_groundHolder,_weapons] call ExileServer_util_fill_fillWeapons;
		[_groundHolder,_containers] call ExileServer_util_fill_fillContainers;
		_vehicleObject call ExileClient_util_containerCargo_clear;
	};

	/// AssetManagement:
	private _hitpoints = [];
	private _allHitPointsArray = getAllHitPointsDamage _vehObject;
	private _hitPointsNamesArray = _allHitPointsArray select 0;
	private _selectionsNamesArray = _allHitPointsArray select 1;
	private _damageValuesArray = _allHitPointsArray select 2;
	{
		_hitPointName = _hitPointsNamesArray select _forEachIndex;
		_damage = _damageValuesArray select _forEachIndex;
		_hitpoints pushback [_hitPointName, _damage];
		diag_log format["DAMAGE: _hitPointName %1 %2",_hitPointName ,_damage];
	} forEach _hitPointsNamesArray;
	diag_log format["DAMAGE: _hitpoints %1",_hitpoints];
	private _xData = _hitpoints;
	_objVehId = _vehicleObject getVariable ["ExileDatabaseID", -1];
	_flagId = _flagObject getVariable ["ExileDatabaseID", -1];

	diag_log format["storeVehicleRequest _xData %1, %2",_xData, typeName _xData];
	diag_log format["storeVehicleRequest _objVehId %1, %2",_objVehId, typeName _objVehId];
	diag_log format["storeVehicleRequest _flagId %1, %2",_flagId, typeName _flagId];

	[_sessionID, "PlayerVehiclesUpdateResponse", [["store", _objVehId, _flagId, _vehicleObject, _xData]]] call ExileServer_system_network_send_to;
	/// END AssetManagement

	_storedVehicles pushBack [typeOf(_vehicleObject), _nickname];
	_flagObject setVariable ["ExileTerritoryStoredVehicles", _storedVehicles, true];
	_vehicleObject call ExileServer_object_vehicle_database_update;
	format["storeVehicle:%1:%2:%3", _flagObject getVariable ["ExileDatabaseID", -1], _nickname, _vehicleObject getVariable ["ExileDatabaseID", -1]] call ExileServer_system_database_query_fireAndForget;
	_vehicleObject call ExileServer_system_vehicleSaveQueue_removeVehicle;
	_vehicleObject call ExileServer_system_simulationMonitor_removeVehicle;
	deleteVehicle _vehicleObject;
	[_sessionID, "storeVehicleResponse", [true,_nickname]] call ExileServer_system_network_send_to;
}
catch
{
	[_sessionID, "storeVehicleResponse", [false,_exception]] call ExileServer_system_network_send_to;
	_exception call ExileServer_util_log;	
};
_playerObject setVariable ["ExileMutex", false];
if !(isNull _vehicleObject) then 
{
	_vehicleObject setVariable ["ExileMutex", false];
};
true