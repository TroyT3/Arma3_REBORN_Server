/*
    JohnO_script_createPlayerActions.sqf
...
// private _BSF_Reborn_IsWounded = player getVariable ["BSF_Reborn_IsWounded",false];
        ...
        if (random 1 > 0.5) then
        {
            ...
            if ([player,_item] call ExileClient_util_playerCargo_canAdd) then
            {
                ...
                ["InfoTitleAndText",
                    [
                        "Item found",
                        format ["I have found a %1 .. It's been placed in your inventory.",_itemDisplayName]
                    ]
                ] call ExileClient_gui_toaster_addTemplateToast;
                if (random 1 > 0) then
                {
